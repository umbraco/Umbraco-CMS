    ...
    public class PublishedContentQuery : IPublishedContentQuery, IPublishedContentQuery2
    {
        ...
        public IEnumerable<PublishedSearchResult> Search(string term, int skip, int take, out long totalRecords, string culture = "*", string indexName = Constants.UmbracoIndexes.ExternalIndexName,ISet<string> loadedFields = null)
        {
            ...
            if(skip == 0 && take == 0)
            {
                ...
                //if (results is ISearchResults2 results2)
                //{
                //    searchResults = results.SkipTake(0);
                //}
                //else
                //{
                    searchResults = results.Skip(0);
                //}
            }
            else
            {
                results = queryExecutor.Execute(take);
                //if (results is ISearchResults2 results2)
                //{
                //    searchResults = results.SkipTake(skip,take);
                //}
                //else
                //{
            ...
            return new CultureContextualSearchResults(searchResults.ToPublishedSearchResults(_publishedSnapshot.Content), _variationContextAccessor, culture);
        }
        /// <inheritdoc />
