    ...
    public class publishingService
    {
        ...
        public static void CheckPublishing(object sender)
        {
            ...
            try
            {
                ...
                if (UmbracoSettings.ChangePublishedStateOfDocumentsForReleaseAndExpiration && !content.Instance.isInitializing)
                {
                    foreach (var d in Document.GetDocumentsForRelease())
                    {
                        try
                        {
                            if(UmbracoSettings.ChangeCreateDateToRelease)
                                d.CreateDateTime = d.ReleaseDate;
