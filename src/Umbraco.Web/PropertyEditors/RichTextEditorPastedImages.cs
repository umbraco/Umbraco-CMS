...
namespace Umbraco.Web.PropertyEditors
{
    public sealed class RichTextEditorPastedImages
    {
        ...
        public RichTextEditorPastedImages(ILogger logger, IMediaService mediaService)
        {
        ...
        private void PersistMediaItem(Guid mediaParentFolder, int userId, HtmlNode img, string tmpImgPath)
        {
            ...
            if (UploadedImages.ContainsKey(tmpImgPath) == false)
            {
                ...
                using (fileStream)
                {
                    mediaFile.SetValue(Constants.Conventions.Media.File, safeFileName, fileStream);
                }
            ...
            var mediaTyped = UmbracoContext.Current.MediaCache.GetById(udi.Guid);
            if (mediaTyped == null)
                throw new Exception($"Could not find media by id {udi.Guid} or there was no UmbracoContext available.");
            ...
            {
                location = _imageUrlGenerator.GetImageUrl(new ImageUrlGenerationOptions(location) { ImageCropMode = "max", Width = width, Height = height });
            }
            ...
            if (UploadedImages.ContainsKey(tmpImgPath) == false)
            {
                ...
                {
                    _logger.Error(GetType(), $"Could not delete temp file or folder {absoluteTempImagePath}", ex);
                }
            }
        }
    }
