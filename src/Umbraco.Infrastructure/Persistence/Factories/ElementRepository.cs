...
public class ElementRepository : ContentRepositoryBase<int, IElement, ElementRepository>, IElementRepository
{
            ...
            propertyEditors, dataValueReferenceFactories, dataTypeService, eventAggregator)
    {
        ...
        _elementByGuidReadRepository = new ElementByGuidReadRepository(this, scopeAccessor, appCaches,
            loggerFactory.CreateLogger<ElementByGuidReadRepository>());
    }
    ...
    private IEnumerable<IElement> MapDtosToContent(List<ElementDto> dtos,
        ...
        bool loadVariants = true)
    {
        ...
        if (loadVariants)
        {
            ...
            if (temps.Count > 0)
            {
                ...
                IDictionary<int, List<ElementVariation>> elementVariations = GetElementVariations(temps);
                foreach (TempContent<IElement> temp in temps)
                {
    ...
    private IElement MapDtoToContent(ElementDto dto)
    {
        ...
        try
        {
            ...
            if (contentType?.VariesByCulture() ?? false)
            {
                ...
                IDictionary<int, List<ElementVariation>> elementVariations = GetElementVariations(ltemp);
    ...
    private void SetVariations(IElement? element, IDictionary<int, List<ContentVariation>> contentVariations,
        IDictionary<int, List<ElementVariation>> elementVariations)
    {
        if (element is null)
        {
            return;
        ...
        if (element.PublishedState is PublishedState.Published && element.PublishedVersionId > 0 && contentVariations.TryGetValue(element.PublishedVersionId, out contentVariation))
        {
            foreach (ContentVariation v in contentVariation)
            {
        ...
        if (elementVariations.TryGetValue(element.Id, out List<ElementVariation>? elementVariation))
        {
            element.SetCultureEdited(elementVariation.Where(x => x.Edited).Select(x => x.Culture));
        }
    }
    ...
    private IDictionary<int, List<ElementVariation>> GetElementVariations<T>(List<TempContent<T>> temps)
        where T : class, IContentBase
    {
        IEnumerable<int> ids = temps.Select(x => x.Id);

        IEnumerable<ElementCultureVariationDto> dtos = Database.FetchByGroups<ElementCultureVariationDto, int>(ids,
            Constants.Sql.MaxParameterCount, batch =>
                Sql()
                    .Select<ElementCultureVariationDto>()
                    .From<ElementCultureVariationDto>()
                    .WhereIn<ElementCultureVariationDto>(x => x.NodeId, batch));

        var variations = new Dictionary<int, List<ElementVariation>>();

        foreach (ElementCultureVariationDto dto in dtos)
        {
            if (!variations.TryGetValue(dto.NodeId, out List<ElementVariation>? variation))
            {
                variations[dto.NodeId] = variation = new List<ElementVariation>();
            }

            variation.Add(new ElementVariation
            {
    ...
    private class ElementVariation
    {
        public string? Culture { get; set; }
    ...
    public override void DeleteVersion(int versionId)
    {
        ...
        SqlTemplate template = SqlContext.Templates.Get("Umbraco.Core.ElementRepository.GetVersion", tsql =>
            tsql.Select<ContentVersionDto>()
                .AndSelect<ElementVersionDto>()
                .From<ContentVersionDto>()
                .InnerJoin<ElementVersionDto>()
                .On<ContentVersionDto, ElementVersionDto>((c, d) => c.Id == d.Id)
                .Where<ContentVersionDto>(x => x.Id == SqlTemplate.Arg<int>("versionId"))
        );
        ElementVersionDto? versionDto =
            Database.Fetch<ElementVersionDto>(template.Sql(new {versionId})).FirstOrDefault();
    ...
    public override void DeleteVersions(int nodeId, DateTime versionDate)
    {
        ...
        SqlTemplate template = SqlContext.Templates.Get("Umbraco.Core.ElementRepository.GetVersions", tsql =>
            tsql.Select<ContentVersionDto>()
                ...
                .Where<ElementVersionDto>(x => !x.Published)
        );
    ...
    protected override void PerformDeleteVersion(int id, int versionId)
    {
        ...
        Database.Delete<ElementVersionDto>("WHERE id = @versionId", new {versionId});
    ...
    protected override void PersistNewItem(IElement entity)
    {
        ...
        if (publishing)
        {
            elementVersionDto.Published = true;
        }

        Database.Insert(elementVersionDto);
        ...
        if (publishing)
        {
            ...
            elementVersionDto.Id = entity.VersionId;
            elementVersionDto.Published = false;
            Database.Insert(elementVersionDto);
        }
        ...
        //}
        //if (Database.ExecuteScalar<int>($"SELECT COUNT(*) FROM {Constants.DatabaseSchema.Tables.ElementVersion} JOIN {Constants.DatabaseSchema.Tables.ContentVersion} ON {Constants.DatabaseSchema.Tables.ElementVersion}.id={Constants.DatabaseSchema.Tables.ContentVersion}.id WHERE [current]=1 AND nodeId=" + content.Id) > 1)
        //{
        //    Debugger.Break();
        //    throw new Exception("oops");
        //}
    ...
    protected override void PersistUpdatedItem(IElement entity)
    {
        ...
        if (!isMoving)
        {
            ...
            if (publishing && entity.PublishedVersionId > 0)
            {
                ...
                Database.Execute(Sql().Update<ElementVersionDto>(u => u.Set(x => x.Published, false))
                    .Where<ElementVersionDto>(x => x.Id == entity.PublishedVersionId));
            }
        ...
        if (!isMoving)
        {
            ...
            Database.Update(elementVersionDto);

            // and, if publishing, insert new content & element version dtos
            if (publishing)
            {
                entity.PublishedVersionId = entity.VersionId;
                ...
                Database.Insert(contentVersionDto);
                entity.VersionId = elementVersionDto.Id = contentVersionDto.Id; // get the new id

                elementVersionDto.Published = false; // non-published version
                Database.Insert(elementVersionDto);
            }
            ...
            // we have lost the publishedValue on IElement (in memory vs database) so we cannot correctly make that comparison.
            //
            ...
            if (entity.ContentType.VariesByCulture())
            {
                ...
                // replace the element version variations (rather than updating)
                Sql<ISqlContext> deleteElementVariations = Sql().Delete<ElementCultureVariationDto>()
                    .Where<ElementCultureVariationDto>(x => x.NodeId == entity.Id);
                Database.Execute(deleteElementVariations);
    ...
    public IElement? Get(Guid id) => _elementByGuidReadRepository.Get(id);

    IEnumerable<IElement> IReadRepository<Guid, IElement>.GetMany(params Guid[]? ids) =>
        _elementByGuidReadRepository.GetMany(ids);

    public bool Exists(Guid id) => _elementByGuidReadRepository.Exists(id);
    ...
    private class ElementByGuidReadRepository : EntityRepositoryBase<Guid, IElement>
    {
        private readonly ElementRepository _outerRepo;

        public ElementByGuidReadRepository(ElementRepository outerRepo, IScopeAccessor scopeAccessor, AppCaches cache,
            ILogger<ElementByGuidReadRepository> logger)
        ...
        protected override IElement? PerformGet(Guid id)
        {
            ...
            IElement element = _outerRepo.MapDtoToContent(dto);
    ...
    private void SanitizeNames(IElement content, bool publishing)
    {
    ...
    private void EnsureInvariantNameExists(IElement content)
    {
    ...
    private void EnsureInvariantNameIsUnique(IElement content) =>
    ...
    private void EnsureVariantNamesAreUnique(IElement content, bool publishing)
    {
