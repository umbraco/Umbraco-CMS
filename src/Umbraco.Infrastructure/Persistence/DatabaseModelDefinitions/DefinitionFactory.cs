        ...
        if (string.IsNullOrEmpty(attribute.ForColumns) == false)
        {
            ...
            foreach (var column in columns)
            {
                ...
                if(sortDirectionSplit.Length == 2)
                {
                    ...
                    {
                        definition.Columns.Add(new IndexColumnDefinition { Name = sortDirectionSplit[0].Trim(), Direction = Direction.Descending });
                    }
                    else if ("ASC".Equals(sortDirectionSplit[1], StringComparison.InvariantCultureIgnoreCase))
                    {
                        definition.Columns.Add(new IndexColumnDefinition { Name = sortDirectionSplit[0].Trim(), Direction = Direction.Ascending });
                    }
                    else
                    {
