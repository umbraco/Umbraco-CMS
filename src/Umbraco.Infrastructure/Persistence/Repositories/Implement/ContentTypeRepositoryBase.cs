...
AND umbracoNode.id <> @id",
    ...
    private void Clear301Redirects(IContentTypeComposition contentType)
    {
        ...
        Sql<ISqlContext> sqlDelete = Sql()
            ...
            .WhereIn((Expression<Func<RedirectUrlDto, object?>>)(x => x.ContentKey),
                sqlSelect);
    ...
    public string GetUniqueAlias(string alias)
    {
        ...
        List<string>? aliases = Database.Fetch<string>(@"SELECT cmsContentType." + aliasColumn + @" FROM cmsContentType
INNER JOIN umbracoNode ON cmsContentType.nodeId = umbracoNode.id
