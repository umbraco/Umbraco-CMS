...
internal class DomainRepository : EntityRepositoryBase<int, IDomain>, IDomainRepository
{
    ...
    {
        var exists = Database.ExecuteScalar<int>("SELECT COUNT(*) FROM umbracoDomain WHERE domainName = @domainName",
        ...
        {
            var languageExists = Database.ExecuteScalar<int>("SELECT COUNT(*) FROM umbracoLanguage WHERE id = @id",
    ...
    internal class DomainModelFactory
    {
        public IDomain BuildEntity(DomainDto dto)
        {
            ...
            {
                Id = dto.Id, LanguageId = dto.DefaultLanguage, RootContentId = dto.RootStructureId
            };
