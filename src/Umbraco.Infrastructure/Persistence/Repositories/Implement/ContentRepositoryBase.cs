...
public abstract class ContentRepositoryBase<TId, TEntity, TRepository> : EntityRepositoryBase<TId, TEntity>,
    ...
    where TRepository : class, IRepository
{
    ...
    public ContentDataIntegrityReport CheckDataIntegrity(ContentDataIntegrityReportOptions options)
    {
        ...
        var rootIds = new[]
        {
            Constants.System.Root, Constants.System.RecycleBinContent, Constants.System.RecycleBinMedia
        };
        ...
        foreach (NodeDto? node in Database.Query<NodeDto>(sql))
        {
            ...
            var pathParts = node.Path.Split(Constants.CharArrays.Comma)
                .Where(x => !rootIds.Contains(int.Parse(x, CultureInfo.InvariantCulture))).ToArray();
            ...
            if (!prevParentIds.Contains(node.ParentId))
            {
                ...
                report.Add(node.NodeId,
                    new ContentDataIntegrityReportEntry(ContentDataIntegrityReport.IssueType
            ...
            else if (pathParts.Length == 0)
            {
                // invalid path
                report.Add(node.NodeId,
                    new ContentDataIntegrityReportEntry(ContentDataIntegrityReport.IssueType.InvalidPathEmpty));
                AppendNodeToFix(nodesToRebuild, node);
            }
            else if (pathParts.Length != node.Level)
            {
                ...
                report.Add(node.NodeId,
                    new ContentDataIntegrityReportEntry(ContentDataIntegrityReport.IssueType.InvalidPathLevelMismatch));
                AppendNodeToFix(nodesToRebuild, node);
            }
            else if (pathParts[pathParts.Length - 1] != node.NodeId.ToString())
            {
                // invalid path
                report.Add(node.NodeId,
                    new ContentDataIntegrityReportEntry(ContentDataIntegrityReport.IssueType.InvalidPathById));
                AppendNodeToFix(nodesToRebuild, node);
            }
            else if (!rootIds.Contains(node.ParentId) && pathParts[pathParts.Length - 2] != node.ParentId.ToString())
            {
                // invalid path
                report.Add(node.NodeId,
                    new ContentDataIntegrityReportEntry(ContentDataIntegrityReport.IssueType.InvalidPathByParentId));
                AppendNodeToFix(nodesToRebuild, node);
            }
            else
            {
        ...
        if (options.FixIssues)
        {
            ...
            foreach ((var nodeId, NodeDto node) in validNodes)
            {
    ...
    protected virtual string ApplySystemOrdering(ref Sql<ISqlContext> sql, Ordering ordering)
    {
        ...
        if (ordering.OrderBy.InvariantEquals("owner"))
        {
            Sql<ISqlContext> joins = Sql()
                .InnerJoin<UserDto>("ownerUser")
                .On<NodeDto, UserDto>((node, user) => node.UserId == user.Id, aliasRight: "ownerUser");

            // see notes in ApplyOrdering: the field MUST be selected + aliased
            sql = Sql(
                InsertBefore(sql, "FROM",
                    ", " + SqlSyntax.GetFieldName<UserDto>(x => x.UserName, "ownerUser") + " AS ordering "),
                sql.Arguments);
        ...
        if (ordering.OrderBy.InvariantEquals("contentTypeAlias"))
        {
            ...
            sql = Sql(
                InsertBefore(sql, "FROM",
                    ", " + SqlSyntax.GetFieldName<ContentTypeDto>(x => x.Alias!, "ctype") + " AS ordering "),
                sql.Arguments);
    ...
    private string ApplyCustomOrdering(ref Sql<ISqlContext> sql, Ordering ordering)
    {
        ...
        Sql<ISqlContext> innerSql = Sql().Select($@"CASE
            ...
            .InnerJoin<PropertyTypeDto>("optype")
            .On<PropertyDataDto, PropertyTypeDto>((pdata, ptype) => pdata.PropertyTypeId == ptype.Id, "opdata",
                "optype")
            .LeftJoin<LanguageDto>()
            .On<PropertyDataDto, LanguageDto>((pdata, lang) => pdata.LanguageId == lang.Id, "opdata")
            ...
            .Where<PropertyDataDto, LanguageDto>(
                (opdata, lang) => opdata.LanguageId == null || lang.IsoCode == ordering.Culture, "opdata");
        ...
        // see notes in ApplyOrdering: the field MUST be selected + aliased
        newSql = InsertBefore(newSql, "FROM",
            ", customPropData.customPropVal AS ordering "); // trailing space is important!
        ...
        where T : class, IContentBase
    {
        ...
        var allPropertyDataDtos = Database.FetchByGroups<PropertyDataDto, int>(versions,
        ...
        IEnumerable<PropertyTypeDto> allPropertyTypeDtos = Database.FetchByGroups<PropertyTypeDto, int>(
            allPropertyTypeIds, Constants.Sql.MaxParameterCount, batch =>
                SqlContext.Sql()
                    .Select<PropertyTypeDto>(r => r.Select(x => x.DataTypeDto))
                    .From<PropertyTypeDto>()
                    .InnerJoin<DataTypeDto>()
                    .On<PropertyTypeDto, DataTypeDto>((left, right) => left.DataTypeId == right.NodeId)
    ...
    private IDictionary<int, PropertyCollection> GetPropertyCollections<T>(List<TempContent<T>> temps,
        IEnumerable<PropertyDataDto> allPropertyDataDtos)
        where T : class, IContentBase
    {
        ...
        foreach (TempContent<T> temp in temps)
        {
            ...
            if (compositionPropertiesIndex.TryGetValue(temp.ContentType.Id,
                    out IPropertyType[]? compositionProperties) == false)
            {
                compositionPropertiesIndex[temp.ContentType.Id] =
                    compositionProperties = temp.ContentType.CompositionPropertyTypes.ToArray();
            }
            ...
            if (temp.VersionId != temp.PublishedVersionId &&
                indexedPropertyDataDtos.TryGetValue(temp.PublishedVersionId,
                    out List<PropertyDataDto>? propertyDataDtos2))
            {
                propertyDataDtos.AddRange(propertyDataDtos2);
            }

            var properties = PropertyFactory.BuildEntities(compositionProperties, propertyDataDtos,
                temp.PublishedVersionId, LanguageRepository).ToList();
            ...
            if (result.ContainsKey(temp.VersionId))
            {
                if (ContentRepositoryBase.ThrowOnWarning)
                {
                    throw new InvalidOperationException(
                        $"The query returned multiple property sets for content {temp.Id}, {temp.ContentType.Name}");
                }

                Logger.LogWarning(
                    "The query returned multiple property sets for content {ContentId}, {ContentTypeName}", temp.Id,
                    temp.ContentType.Name);
            }
    ...
    private string GetAliasedField(string field, Sql sql)
    {
        ...
        Match? match = matches.FirstOrDefault(m => m.Groups[1].Value.InvariantEquals(field));
        return match == null ? field : match.Groups[2].Value;
    }

    protected string GetQuotedFieldName(string tableName, string fieldName) =>
        SqlContext.SqlSyntax.GetQuotedTableName(tableName) + "." + SqlContext.SqlSyntax.GetQuotedColumnName(fieldName);
    ...
    protected void PersistRelations(TEntity entity)
    {
        ...
        var keyToIds = Database.Fetch<NodeIdKey>(Sql().Select<NodeDto>(x => x.NodeId, x => x.UniqueId).From<NodeDto>()
    ...
    /// </remarks>
    protected void InsertPropertyValues(TEntity entity, int publishedVersionId, out bool edited,
        out HashSet<string>? editedCultures)
    {
        ...
        IEnumerable<PropertyDataDto> propertyDataDtos = PropertyFactory.BuildDtos(entity.ContentType.Variations,
            entity.VersionId, publishedVersionId, entity.Properties, LanguageRepository, out edited,
            out editedCultures);
    ...
    /// <param name="edited"></param>
    /// <param name="editedCultures"></param>
    protected void ReplacePropertyValues(TEntity entity, int versionId, int publishedVersionId, out bool edited,
        out HashSet<string>? editedCultures)
    {
        ...
        Sql<ISqlContext> propDataSql = SqlContext.Sql().Select("*").From<PropertyDataDto>()
            .Where<PropertyDataDto>(x => x.VersionId == versionId).ForUpdate();
        List<PropertyDataDto>? existingPropData = Database.Fetch<PropertyDataDto>(propDataSql);
        var propertyTypeToPropertyData =
            new Dictionary<(int propertyTypeId, int versionId, int? languageId, string? segment), PropertyDataDto>();
        ...
        foreach (PropertyDataDto propertyDataDto in propertyDataDtos)
        {
            ...
            if (propertyTypeToPropertyData.TryGetValue(
                    (propertyDataDto.PropertyTypeId, propertyDataDto.VersionId, propertyDataDto.LanguageId,
                        propertyDataDto.Segment), out PropertyDataDto? propData))
            {
        ...
        {
            Database.Execute(SqlContext.Sql().Delete<PropertyDataDto>()
                .WhereIn<PropertyDataDto>(x => x.Id, existingPropDataIds));
        }
    }

    private class NodeIdKey
    {
        [Column("id")] public int NodeId { get; set; }

        [Column("uniqueId")] public Guid UniqueId { get; set; }
    }
    ...
    {
        SqlTemplate template = SqlContext.Templates.Get(Constants.SqlTemplates.VersionableRepository.GetVersionIds,
            tsql =>
    ...
    public virtual void DeleteVersion(int versionId)
    {
        ...
        SqlTemplate template = SqlContext.Templates.Get(Constants.SqlTemplates.VersionableRepository.GetVersion, tsql =>
            tsql.Select<ContentVersionDto>().From<ContentVersionDto>()
                .Where<ContentVersionDto>(x => x.Id == SqlTemplate.Arg<int>("versionId"))
        );
        ContentVersionDto? versionDto =
            Database.Fetch<ContentVersionDto>(template.Sql(new {versionId})).FirstOrDefault();
    ...
    public virtual void DeleteVersions(int nodeId, DateTime versionDate)
    {
        ...
        SqlTemplate template = SqlContext.Templates.Get(Constants.SqlTemplates.VersionableRepository.GetVersions,
            tsql =>
                tsql.Select<ContentVersionDto>().From<ContentVersionDto>().Where<ContentVersionDto>(x =>
    ...
    protected void SetEntityTags(IContentBase entity, ITagRepository tagRepo, IJsonSerializer serializer)
    {
        foreach (IProperty property in entity.Properties)
        {
            ...
            if (property.PropertyType.VariesByCulture())
            {
                ...
                {
                    IEnumerable<string> tagsValue =
                        property.GetTagsValue(PropertyEditors, DataTypeService, serializer, pvalue.Culture);
                    var languageId = LanguageRepository.GetIdByIsoCode(pvalue.Culture);
                    IEnumerable<Tag> cultureTags = tagsValue.Select(x =>
                        new Tag {Group = tagConfiguration.Group, Text = x, LanguageId = languageId});
            ...
            {
                IEnumerable<string>
                    tagsValue = property.GetTagsValue(PropertyEditors, DataTypeService, serializer); // strings
    ...
    /// </summary>
    protected void ClearEntityTags(IContentBase entity, ITagRepository tagRepo) => tagRepo.RemoveAll(entity.Id);
    ...
    protected void OnUowRemovingEntity(IContentBase entity) =>
        _eventAggregator.Publish(new ScopedEntityRemoveNotification(entity, new EventMessages()));
    ...
    protected virtual string? EnsureUniqueNodeName(int parentId, string? nodeName, int id = 0)
    {
        SqlTemplate template = SqlContext.Templates.Get(
            Constants.SqlTemplates.VersionableRepository.EnsureUniqueNodeName, tsql => tsql
    ...
    {
        SqlTemplate template = SqlContext.Templates.Get(Constants.SqlTemplates.VersionableRepository.GetSortOrder,
            tsql => tsql
    ...
    {
        SqlTemplate template = SqlContext.Templates.Get(Constants.SqlTemplates.VersionableRepository.GetParentNode,
            tsql => tsql
                .Select<NodeDto>()
                .From<NodeDto>()
    ...
    {
        SqlTemplate template = SqlContext.Templates.Get(Constants.SqlTemplates.VersionableRepository.GetReservedId,
            tsql => tsql
                .Select<NodeDto>(x => x.NodeId)
                .From<NodeDto>()
                .Where<NodeDto>(x =>
                    x.UniqueId == SqlTemplate.Arg<Guid>("uniqueId") &&
    ...
    public virtual IEnumerable<TEntity>? GetRecycleBin() => Get(Query<TEntity>().Where(entity => entity.Trashed));
