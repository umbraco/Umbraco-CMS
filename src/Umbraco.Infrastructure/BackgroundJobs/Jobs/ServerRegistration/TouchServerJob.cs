...
public class TouchServerJob : RecurringBackgroundJobBase
{
    ...
    public override async Task RunJobAsync(CancellationToken cancellationToken)
    {
        ...
        // and silently stop server-registration heartbeats until the process is recycled. BoundedServerTouch
        // offloads it to the thread pool and bounds the wait so the loop survives and keeps touching; it hands
        // back the raw, unbounded task so we can keep detecting an overlapping stalled attempt below.
        // (See InstructionProcessJob for the same offload pattern and the ExecutionContext.SuppressFlow rationale.)
        _inFlightTouch = await BoundedServerTouch.TryTouchAsync(
