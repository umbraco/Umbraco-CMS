...
public class RuntimeState : IRuntimeState
{
    ...
    public void DetermineRuntimeLevel()
    {
        ...
        if (_conflictingRouteService.HasConflictingRoutes(out var controllerName))
        {
            Level = RuntimeLevel.BootFailed;
            Reason = RuntimeLevelReason.BootFailedOnException;
            BootFailedException =
                new BootFailedException(
                    $"Conflicting routes, you cannot have multiple controllers with the same name: {controllerName}");
            ...
            if (_unattendedSettings.Value.PackageMigrationsUnattended)
                ...
                else
                {
                    _logger.LogInformation(
                        "Package migrations need to execute but unattended package migrations is disabled. They will need to be run from the back office.");
    ...
    private UmbracoDatabaseState GetUmbracoDatabaseState(IUmbracoDatabaseFactory databaseFactory)
    {
        try
        {
            ...
            using (IUmbracoDatabase database = databaseFactory.CreateDatabase())
            {
                ...
                IReadOnlyList<string> packagesRequiringMigration =
