...
public class GridPropertyEditor : DataEditor
{
    ...
    protected override IDataValueEditor CreateValueEditor() =>
        DataValueEditorFactory.Create<GridPropertyValueEditor>(Attribute!);

    protected override IConfigurationEditor CreateConfigurationEditor() =>
        new GridConfigurationEditor(_ioHelper, _editorConfigurationParser);
    ...
    internal class GridPropertyValueEditor : DataValueEditor, IDataValueReference
    {
        ...
        public IEnumerable<UmbracoEntityReference> GetReferences(object? value)
        {
            ...
            if (macroValues is not null)
            {
                foreach (UmbracoEntityReference umbracoEntityReference in _macroParameterParser
                             .FindUmbracoEntityReferencesFromGridControlMacros(macroValues))
                {
                    yield return umbracoEntityReference;
                }
            }
        ...
        public override object? ToEditor(IProperty property, string? culture = null, string? segment = null)
        {
            ...
            GridValue? grid = DeserializeGridValue(val!, out IEnumerable<GridValue.GridControl>? rtes, out _, out _);
            ...
            out IEnumerable<GridValue.GridControl>? mediaValues, out IEnumerable<GridValue.GridControl>? macroValues)
        {
            ...
            GridValue.GridControl[]? controls = grid?.Sections
                .SelectMany(x => x.Rows.SelectMany(r => r.Areas).SelectMany(a => a.Controls)).ToArray();
