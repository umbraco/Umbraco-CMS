...
if (swaggerUrl === undefined || outputDir === undefined) {
  ...
  console.error(
    `Example: node generate-openapi.js ${chalk.yellow('https://localhost:44324/umbraco/swagger/Search/swagger.json')} ${chalk.yellow('src/settings/api')}`,
  );
  process.exit(1);
}
...
fetch(swaggerUrl)
  .then(async (response) => {
    if (!response.ok) {
      ...
      console.error(
        `Please verify or change the URL in the ${chalk.yellow('package.json')} for the script ${chalk.yellow('generate-client')}`,
      );
      return;
    }

    console.log(`OpenAPI spec fetched successfully`);
    console.log(`Calling ${chalk.yellow('hey-api')} to generate TypeScript client to ${chalk.yellow(outputDir)}`);
