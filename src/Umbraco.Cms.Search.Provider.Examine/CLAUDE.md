...
- Imports `UMB_SEARCH_WORKSPACE_CONTEXT` from `@umbraco-cms/backoffice/search-management` (via tsconfig path mapping)
...
- A generated alias for every `@umbraco-cms/backoffice/<sub>` export, mapped into the in-repo backoffice sources (`../../Umbraco.Web.UI.Client/src/...`) — there is **no npm dependency** on `@umbraco-cms/backoffice`. This includes `@umbraco-cms/backoffice/search-management`, which resolves to `../../Umbraco.Web.UI.Client/src/packages/search-management/index.ts` and is how this client reaches `UMB_SEARCH_WORKSPACE_CONTEXT` (the search index management UI now lives inside the backoffice client itself, not in a separate sibling project)

Vite externalizes all `@umbraco*` imports at build time; the importmap resolves them at runtime. Re-run `npm run generate:tsconfig` whenever the backoffice `exports` map changes and commit the result.

The backoffice's ambient extension types are pulled in by `src/backoffice-types.d.ts` (a side-effect type import), because `compilerOptions.types` cannot resolve through path aliases.
...
| tsconfig paths | Yes (logical imports) | Yes (generated aliases for every `@umbraco-cms/backoffice/<sub>` export, including `search-management`) |
