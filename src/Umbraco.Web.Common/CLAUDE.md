...
**Convention**: middleware lives in `Middleware/` as a class implementing `IMiddleware` (not `Microsoft.AspNetCore.Builder.UseExtensions.Use(...)` inline lambdas). The class is registered as a singleton in `AddWebComponents` (`UmbracoBuilderExtensions.cs` ~line 336–340) and wired into the pipeline via `app.UseMiddleware<TMiddleware>()`. Any companion extension method on `IApplicationBuilder` should be a thin one-line wrapper around `UseMiddleware<T>()` — do not write `builder.Use(async (ctx, next) => …)` inline in extension methods; it bypasses DI and breaks the project's testing pattern. Per-request state goes in `InvokeAsync` locals; per-app state computed from injected services goes in the constructor (both `IBackOfficePathGenerator` and `IHostingEnvironment` are singletons, for example).
