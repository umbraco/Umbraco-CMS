...
**Convention**: middleware lives in `Middleware/` as a class implementing `IMiddleware`. Every middleware in the codebase (BootFailed, UmbracoRequest, UmbracoRequestLogging, PreviewAuthentication, ProtectRecycleBinMedia, UmbracoBackOfficeCacheHeaders here; BackOfficeAuthorizationInitialization, BackOfficeExternalLoginProviderError, UnhandledExceptionLogger in Api.Management; BasicAuthentication in Web.Website) follows this pattern: the class is registered as a singleton in `AddWebComponents` (`UmbracoBuilderExtensions.cs` ~line 336–341) and wired into the pipeline via `app.UseMiddleware<TMiddleware>()`. Any companion extension method on `IApplicationBuilder` should be a thin one-line wrapper around `UseMiddleware<T>()` — do not write `builder.Use(async (ctx, next) => …)` inline in extension methods for new middleware; it bypasses DI and breaks the project's testing pattern (`IMiddleware` is mockable/registerable; an inline lambda is neither). Per-request state goes in `InvokeAsync` locals; per-app state computed from injected services goes in the constructor (both `IBackOfficePathGenerator` and `IHostingEnvironment` are singletons, for example). Two small inline-lambda exceptions exist (`CspNonceExtensions.UseUmbracoCspNonceInjection` and the dev-only `WebApplicationExtensions.UseDocumentedContentSecurityPolicy`); both are tiny options-pattern helpers without per-request DI state — not a precedent for new work.
