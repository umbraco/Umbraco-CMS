...
internal class InMemoryModelFactory : IAutoPublishedModelFactory, IRegisteredObject, IDisposable
{
        ...
        try
        {
            ...
            using (_profilingLogger.DebugDuration<InMemoryModelFactory>("Get models.", "Got models."))
            {
                try
                {
                    ...
                    IEnumerable<Type> types = assembly.ExportedTypes.Where(x =>
                        x.Inherits<PublishedContentModel>() || x.Inherits<PublishedElementModel>());
    ...
    private Assembly ReloadAssembly(string pathToAssembly)
    {
        ...
        if (!(_currentAssemblyLoadContext is null))
        {
            ...
            ApplicationPart? currentPart =
                _applicationPartManager.ApplicationParts.FirstOrDefault(x =>
                    x.Name == RoslynCompiler.GeneratedAssemblyName);
            if (currentPart != null)
            {
    ...
    private Assembly GetModelsAssembly(bool forceRebuild)
    {
            ...
            if (File.Exists(dllPathFile))
            {
                ...
                if (File.Exists(dllPath) && !File.Exists(dllPath + ".delete"))
                {
                    ...
                    ModelsBuilderAssemblyAttribute?
                        attr = assembly.GetCustomAttribute<ModelsBuilderAssemblyAttribute>();
        ...
        _ver++;
        var mbAssemblyDirective = $@"[assembly:ModelsBuilderAssembly(IsInMemory = true, SourceHash = ""{currentHash}"")]
