...
// ReSharper restore IntroduceOptionalParameters.Local
            ...
            else
            {
                ...
                if (content != null)
                {
                    ...
                    if (content != null && s.Count < _maxDepth)
                    {
                        ...
                        while (content != null)
                        {
                            InternalState = new State(content, InternalState, InternalState.Content?.ChildIds,
                                InternalState.Content?.ChildIds?.IndexOf(content.Id) ?? -1, StatePosition.Element);
    ...
    public class State
    {
        ...
        public State(INavigableContent? content, State? parent, IList<int>? siblings, int siblingIndex,
