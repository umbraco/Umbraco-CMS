...
public class DocumentUrlService : IDocumentUrlService, IMemoryCacheSizeReporter
{
    ...
    /// database-writing methods below (<see cref="CreateOrUpdateUrlSegmentsAsync(IEnumerable{IContent})"/> and
    /// friends) reacts to a purely local, in-process notification fired on the server that made the edit - never
    /// something routed from another server's cache-refresh instruction - so an unresolved role here almost
    /// always means "this server originated the change and must persist it". Skipping would silently drop the
    /// write instead.
