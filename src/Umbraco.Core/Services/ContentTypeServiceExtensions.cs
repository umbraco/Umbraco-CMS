...
public static class ContentTypeServiceExtensions
{
    ...
    /// </remarks>
    public static async Task<IEnumerable<TItem>> GetComposedOfAsync<TItem>(
        this IAsyncContentTypeBaseService<TItem> contentTypeBaseService,
        int id,
        ComposedOfType composedOfType)
        where TItem : IContentTypeComposition
    {
        IEnumerable<TItem> composedOf = await contentTypeBaseService.GetComposedOfAsync(id);

        return composedOfType switch
        {
            ComposedOfType.Composition => composedOf.Where(x => x.ParentId != id),
            ComposedOfType.Inheritance => composedOf.Where(x => x.ParentId == id),
            _ => composedOf,
        };
    }
