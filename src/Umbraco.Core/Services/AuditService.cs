...
namespace Umbraco.Core.Services
{
    public sealed class AuditService : ScopeRepositoryService, IAuditService
    {
        ...
        public IEnumerable<AuditItem> GetLogs(int objectId)
        {
            ...
            {
                var repo = uow.CreateRepository<IAuditRepository>();
                var result = repo.GetByQuery(uow.Query<AuditItem>().Where(x => x.Id == objectId));
                uow.Complete();
                return result;
            }
        }
        ...
        public IEnumerable<AuditItem> GetUserLogs(int userId, AuditType type, DateTime? sinceDate = null)
        {
            using (var uow = UowProvider.CreateUnitOfWork())
            {
                ...
                var result = sinceDate.HasValue == false
                    ? repo.GetByQuery(uow.Query<AuditItem>().Where(x => x.UserId == userId && x.AuditType == type))
                    : repo.GetByQuery(uow.Query<AuditItem>().Where(x => x.UserId == userId && x.AuditType == type && x.CreateDate >= sinceDate.Value));
                uow.Complete();
                return result;
            }
        }
        ...
        public IEnumerable<AuditItem> GetLogs(AuditType type, DateTime? sinceDate = null)
        {
            using (var uow = UowProvider.CreateUnitOfWork())
            {
                var repo = uow.CreateRepository<IAuditRepository>();
                var result = sinceDate.HasValue == false
                    ? repo.GetByQuery(uow.Query<AuditItem>().Where(x => x.AuditType == type))
                    : repo.GetByQuery(uow.Query<AuditItem>().Where(x => x.AuditType == type && x.CreateDate >= sinceDate.Value));
                uow.Complete();
                return result;
            }
        }
