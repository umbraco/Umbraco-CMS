...
public class UrlProvider : IPublishedUrlProvider
{
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="UrlProvider" /> class with an Umbraco context and a list of URL
    ///     providers.
    /// </summary>
    ...
    ///     </para>
    /// </remarks>
    public IEnumerable<UrlInfo> GetOtherUrls(int id, Uri current) => _urlProviders.SelectMany(provider =>
        provider.GetOtherUrls(id, current) ?? Enumerable.Empty<UrlInfo>());
