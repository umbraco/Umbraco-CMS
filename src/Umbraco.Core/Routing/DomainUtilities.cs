...
public static class DomainUtilities
{
    ...
    ///     <para>
    ///         If <paramref name="uri" /> is null, pick the first domain that matches <paramref name="culture" />,
    ///         else the first that matches <paramref name="defaultCulture" />, else the first one (ordered by id), else null.
    ///     </para>
    ///     <para>If <paramref name="uri" /> is not null, look for domains that would be a base uri of the current uri,</para>
    ///     <para>
    ///         If more than one domain matches, then the <paramref name="filter" /> function is used to pick
    ///         the right one, unless it is <c>null</c>, in which case the method returns <c>null</c>.
    ///     </para>
        ...
        string? culture)
    {
        ...
        var baseDomains = domainsAndUris.Where(d => IsBaseOf(d, currentWithSlash) && MatchesCulture(d, culture))
            .ToList();
    ...
    /// <summary>
    ///     Gets a value indicating whether there is another domain defined down in the path to a node under the current
    ///     domain's root node.
    /// </summary>
    /// <param name="domains">The domains.</param>
