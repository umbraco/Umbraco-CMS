...
/// </summary>
/// <typeparam name="TComposer">The type of the composer.</typeparam>
public class ComposerGraph<TComposer>
    where TComposer : ICustomComposer
{
    ...
    private readonly ILogger<ComposerGraph<TComposer>> _logger;

    /// <summary>
    ///     Initializes a new instance of the <see cref="ComposerGraph" /> class.
    /// </summary>
    /// <param name="builder">The composition.</param>
    /// <param name="composerTypes">The <typeparamref name="TComposer"/> types.</param>
    ...
    ///     logger
    /// </exception>
    public ComposerGraph(IUmbracoBuilder builder, IEnumerable<Type> composerTypes, IEnumerable<Attribute> enableDisableAttributes, ILogger<ComposerGraph<TComposer>> logger)
    {
    ...
    public void Compose()
    {
        ...
        foreach (TComposer composer in InstantiateComposers(orderedComposerTypes))
        {
    ...
    internal Dictionary<Type, List<Type>?> GetRequirements(bool throwOnMissing = true)
    {
            ...
            where TAttribute : Attribute
        {
            foreach (TAttribute attribute in type.GetCustomAttributes<TAttribute>())
            {
                ...
                if (typeInAttribute != null && // if the attribute references a type ...
                    ...
                    typeof(TComposer).IsAssignableFrom(typeInAttribute) && // ... which implements TComposer ...
                    !iset.Contains(typeInAttribute)) // ... which is not already in the list
                {
                    ...
                    // add all its interfaces implementing TComposer
                    foreach (Type i in typeInAttribute.GetInterfaces()
                                 .Where(x => typeof(TComposer).IsAssignableFrom(x)))
                    {
                        iset.Add(i);
                        set2.Add(i);
                    }
        ...
        var interfaces = new HashSet<Type>(composerTypeList.SelectMany(x =>
            x.GetInterfaces().Where(y => typeof(TComposer).IsAssignableFrom(y))));
    ...
    private static IEnumerable<TComposer> InstantiateComposers(IEnumerable<Type> types)
    {
        foreach (Type type in types)
        {
            ...
            yield return (TComposer)ctor.Invoke(Array.Empty<object>());
        }
    }
