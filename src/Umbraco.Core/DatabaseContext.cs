    ...
    public class DatabaseContext
    {
        ...
        internal Result UpgradeSchemaAndData()
        {
            try
            {
                ...
                {
                    var semVerVersion = string.Format("{0}.{1}.{2}", versionParts[0], versionParts[1], versionParts[2]);
                    currentVersion = new Version(semVerVersion);
                }
                // Version could be empty or malformed
                if (string.IsNullOrEmpty(configuredVersion) || versionParts.Length < 3)
                {
