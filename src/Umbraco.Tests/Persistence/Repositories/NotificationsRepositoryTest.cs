...
namespace Umbraco.Tests.Persistence.Repositories
{
    ...
    public class NotificationsRepositoryTest : BaseDatabaseFactoryTest
    {
        ...
        public void GetUserNotifications()
        {
            ...
            using (var repo = new NotificationsRepository(unitOfWork))
            {
                var userDto = new UserDto {ContentStartId = -1, Email = "test", Login = "test", MediaStartId = -1, Password = "test", UserName = "test", UserLanguage = "en", CreateDate = DateTime.Now, UpdateDate = DateTime.Now };
        ...
        public void GetEntityNotifications()
        {
            ...
            using (var repo = new NotificationsRepository(unitOfWork))
            {
                ...
                for (var i = 0; i < 10; i++)
                {
                    var userDto = new UserDto {ContentStartId = -1, Email = "test" + i, Login = "test" + i, MediaStartId = -1, Password = "test", UserName = "test" + i, UserLanguage = "en", CreateDate = DateTime.Now, UpdateDate = DateTime.Now};
                    unitOfWork.Database.Insert(userDto);
                    var userNew = Mock.Of<IUser>(e => e.Id == userDto.Id);
