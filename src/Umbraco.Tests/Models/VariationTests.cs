...
namespace Umbraco.Tests.Models
{
    ...
    public class VariationTests
    {
        ...
        public void SetUp()
        {
            ...
            contentTypeMock.Setup(x => x.SupportsPropertyVariation(It.IsAny<string>(), "*", true)).Returns(true);
            contentTypeServiceMock.Setup(x => x.Get(It.IsAny<int>())).Returns(contentTypeMock.Object);
            var serviceContext = new ServiceContext(
                dataTypeService: dataTypeService,
                localizedTextService: Mock.Of<ILocalizedTextService>(),
                contentPublishingService: new ContentPublishingService(contentTypeServiceMock.Object)
                );
            ...
            _contentPublishingService = serviceContext.ContentPublishingService;
        }
        ...
        public void ContentPublishValues()
        {
            ...
            // can publish value
            // and get edited and published values
            Assert.IsTrue(_contentPublishingService.PublishCulture(content));
            Assert.AreEqual("a", content.GetValue("prop"));
            ...
            // can clear value
            _contentPublishingService.UnpublishCulture(content);
            Assert.AreEqual("b", content.GetValue("prop"));
            Assert.IsNull(content.GetValue("prop", published: true));
            ...
            // can publish value
            // and get edited and published values
            Assert.IsFalse(_contentPublishingService.PublishCulture(content, langFr)); // no name
            content.SetCultureName("name-fr", langFr);
            Assert.IsTrue(_contentPublishingService.PublishCulture(content, langFr));
            ...
            // same for culture
            _contentPublishingService.UnpublishCulture(content, langFr);
            Assert.AreEqual("c", content.GetValue("prop", langFr));
            Assert.IsNull(content.GetValue("prop", langFr, published: true));
            _contentPublishingService.PublishCulture(content, langFr);
            Assert.AreEqual("c", content.GetValue("prop", langFr));
            Assert.AreEqual("c", content.GetValue("prop", langFr, published: true));

            _contentPublishingService.UnpublishCulture(content); // clears invariant props if any
            Assert.IsNull(content.GetValue("prop"));
            Assert.IsNull(content.GetValue("prop", published: true));
            _contentPublishingService.PublishCulture(content); // publishes invariant props if any
            Assert.IsNull(content.GetValue("prop"));
            Assert.IsNull(content.GetValue("prop", published: true));
        ...
        public void ContentPublishValuesWithMixedPropertyTypeVariations()
        {
            ...
            Assert.IsFalse(_contentPublishingService.PublishCulture(content, langFr)); // fails because prop1 is mandatory
            ...
            Assert.IsFalse(_contentPublishingService.PublishCulture(content)); // fails because prop2 is mandatory
        ...
        public void ContentPublishVariations()
        {
            ...
            Assert.IsFalse(_contentPublishingService.PublishCulture(content, langFr));
