...
namespace umbraco.cms.businesslogic.propertytype
{
    public class PropertyTypeGroup
    {
		...
		public IEnumerable<PropertyTypeGroup> GetPropertyTypeGroups()
        {
            ...
            using (IRecordsReader dr =
                SqlHelper.ExecuteReader(@"
                    ...
                    WHERE parentGroupId = @id", SqlHelper.CreateParameter("@id", Id))
                )
            {
                while (dr.Read())
                {
        ...
        public void Delete()
        {
            ...
            foreach (PropertyTypeGroup ptg in GetPropertyTypeGroups())
                ptg.Delete();
        ...
        {
            PropertyTypeGroup ptg = new PropertyTypeGroup();
            ptg.Id = id;
            ptg.Load();
            return ptg;
        }
		...
		public static IEnumerable<PropertyTypeGroup> GetPropertyTypeGroupsFromContentType(int contentTypeId)
        {
            ...
            using (IRecordsReader dr =
                SqlHelper.ExecuteReader(@"
                    ...
                    FROM 
                        cmsPropertyTypeGroup 
                    WHERE contenttypeNodeId = @contentTypeId", SqlHelper.CreateParameter("@contentTypeId", contentTypeId))
                )
            {
                while (dr.Read())
                {
