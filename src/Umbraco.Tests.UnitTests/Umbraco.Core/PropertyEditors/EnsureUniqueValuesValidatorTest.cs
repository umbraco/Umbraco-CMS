...
namespace Umbraco.Tests.UnitTests.Umbraco.Core.PropertyEditors
{
    ...
    public class EnsureUniqueValuesValidatorTest
    {
        ...
        {
            var validator = new ValueListUniqueValueValidator();
            var result = validator.Validate(new JArray(JObject.FromObject(new { value = "hello" }), JObject.FromObject(new { value = "world" })), null, new ColorPickerPropertyEditor(_loggerFactory, Mock.Of<IDataTypeService>(), Mock.Of<ILocalizationService>(), Mock.Of<IIOHelper>(), Mock.Of<IShortStringHelper>(), Mock.Of<ILocalizedTextService>(), new JsonNetSerializer()));
            Assert.AreEqual(0, result.Count());
        }
