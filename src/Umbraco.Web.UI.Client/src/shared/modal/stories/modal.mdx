...
### Consume UmbModalManagerContext from an element

The modal manager context can be used to open modals.
...
A modal can be opened by calling the open method on the UmbModalManagerContext. The methods will accept a modal token (or extension alias), an optional dataset, and optional modal options. It returns an instance of UmbModalHandler.
...
class MyElement extends UmbElementMixin(LitElement) {
	...
	#onClick() {
		...
		const modalHandler = this.#modalManagerContext?.open(MY_MODAL_TOKEN), data, options);
