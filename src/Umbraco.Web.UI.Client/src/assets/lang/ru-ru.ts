/** 
* Origin File: https://github.com/umbraco/Umbraco-CMS/tree/v13/contrib/src/Umbraco.Core/EmbeddedResources/Lang/ru.xml 
...
export default{
	...
	assignDomain: {
		...
		setLanguageHelp: "Установите язык (культуру) для всех дочерних узлов,<br /> или унаследуйте язык от родительских узлов.<br />\n\t\tЭта установка будет применена также и к текущему узлу, если только для него ниже явно не задан домен.",
	},
	auditTrails: {
	...
	blueprints: {
		...
		blueprintDescription: "Шаблон содержимого — это предопределенный набор данных, который редактор может использовать для начального заполнения свойств при создании узлов содержимого",
	},
	bulk: {
		done: "Завершено",
	...
	content: {
		...
		expireDate: "Скрыть",
		getUrlException: "ВНИМАНИЕ: невозможно получить URL документа (внутренняя ошибка - подробности в системном журнале)",
		isPublished: "Опубликовано",
		isSensitiveValue: "Это значение скрыто. Если Вам нужен доступ к просмотру этого значения, свяжитесь с администратором веб-сайта.",
		...
		nestedContentEditorNotSupported: "Свойство '%0%' использует редактор '%1%', который не поддерживается для вложенного содержимого.",
		...
		parentNotPublished: "Этот документ опубликован, но скрыт, потому что его родительский документ '%0%' не опубликован",
		parentNotPublishedAnomaly: "ВНИМАНИЕ: этот документ опубликован, но его нет в глобальном кэше (внутренняя ошибка - подробности в системном журнале)",
		publish: "Опубликовать",
		...
		scheduledPublishDocumentation: "<a href=\"https://docs.umbraco.com/umbraco-cms/fundamentals/data/scheduled-publishing#timezones\" target=\"_blank\" rel=\"noopener\">Что это означает?</a>",
		setDate: "Задать дату",
		sortDone: "Порядок сортировки обновлен",
		sortHelp: "Для сортировки узлов просто перетаскивайте узлы или нажмите на заголовке столбца. Вы можете выбрать несколько узлов, удерживая клавиши \"shift\" или \"ctrl\" при пометке",
	...
	contentTypeEditor: {
		...
		enableListViewDescription: "Устанавливает представление документа данного типа в виде сортируемого списка дочерних документов с функцией поиска, в отличие от обычного представления дочерних документов в виде дерева",
		...
		childNodesDescription: "Позволяет указать перечень типов документов, допустимых для создания документов, дочерних к данному типу",
		chooseChildNode: "Выбрать дочерний узел",
		compositionsDescription: "Унаследовать вкладки и свойства из уже существующего типа документов. Вкладки будут либо добавлены в создаваемый тип, либо в случае совпадения названий вкладок будут добавлены наследуемые свойства.",
		compositionInUse: "Этот тип документов уже участвует в композиции другого типа, поэтому сам не может быть композицией.",
		...
		usingThisDocument: ", использующие этот тип документов, будут безвозвратно удалены, пожалуйста, подтвердите это действие.",
		usingThisMedia: ", использующие этот тип медиа, будут безвозвратно удалены, пожалуйста, подтвердите это действие.",
		usingThisMember: ", использующие этот тип участников, будут безвозвратно удалены, пожалуйста, подтвердите это действие.",
		...
		isSensitiveDataDescription: "Скрывает значение это свойства от редакторов содержимого, не имеющих доступа к конфеденциальной информации",
	...
	create: {
		...
		noDocumentTypes: "Нет ни одного разрешенного типа документов для создания. Необходимо разрешить нужные Вам типы в секции \"Установки\" в дереве <strong>\"Типы документов\"</strong>.",
		noMediaTypes: "Нет ни одного разрешенного типа медиа-материалов для создания. Необходимо разрешить нужные Вам типы в секции \"Установки\" в дереве <strong>\"Типы медиа-материалов\"</strong>.",
	...
	dashboard: {
		...
		nothinghappens: "Если административная панель не загружается, Вам, возможно, следует разрешить всплывающие окна для данного сайта",
	...
	defaultdialogs: {
		...
		recycleBinDeleting: "Все элементы в корзине сейчас удаляются. Пожалуйста, не закрывайте это окно до окончания процесса удаления",
		recycleBinIsEmpty: "Корзина пуста",
		recycleBinWarning: "Вы больше не сможете восстановить элементы, удаленные из корзины",
		regexSearchError: "Сервис <a target='_blank' rel='noopener' href='http://regexlib.com'>regexlib.com</a> испытывает в настоящее время некоторые трудности, не зависящие от нас. Просим извинить за причиненные неудобства.",
		regexSearchHelp: "Используйте поиск регулярных выражений для добавления сервиса проверки к полю Вашей формы. Например: 'email, 'zip-code', 'URL'",
		removeMacro: "Удалить макрос",
		requiredField: "Обязательное поле",
		sitereindexed: "Сайт переиндексирован",
		siterepublished: "Кэш сайта был обновлен. Все опубликованное содержимое приведено в актуальное состояние, в то время как неопубликованное содержимое по-прежнему не опубликовано",
		siterepublishHelp: "Кэш сайта будет полностью обновлен. Все опубликованное содержимое будет обновлено, в то время как неопубликованное содержимое по-прежнему останется неопубликованным",
	...
	errorHandling: {
		errorButDataWasSaved: "Ваши данные сохранены, но для того, чтобы опубликовать этот документ, Вы должны сначала исправить следующие ошибки:",
		...
		errorInPasswordFormat: "Пароль должен состоять как минимум из %0% символов, хотя бы %1% из которых не являются буквами",
	...
	errors: {
		...
		codemirroriewarning: "ПРЕДУПРЕЖДЕНИЕ! Несмотря на то, что CodeMirror по-умолчанию разрешен в данной конфигурации, он по-прежнему отключен для браузеров Internet Explorer ввиду нестабильной работы",
		...
		pictureResizeBiggerThanOrg: "Вы пытаетесь увеличить изображение по сравнению с его исходным размером. Уверены, что хотите сделать это?",
	...
	grid: {
		...
		gridLayoutsDetail: "Шаблоны являются рабочим пространством для редактора сетки, обычно Вам понадобится не более одного или двух шаблонов",
	...
	healthcheck: {
		...
		checkErrorMessageDifferentExpectedValue: "Ожидаемое значение '%1%' для параметра '%2%' в файле конфигурации '%3%', найденное значение: '%0%'.",
		checkErrorMessageUnexpectedValue: "Найдено неожиданное значение '%0%' для параметра '%2%' в файле конфигурации '%3%'.",
		macroErrorModeCheckSuccessMessage: "Параметр 'MacroErrors' установлен в '%0%'.",
		macroErrorModeCheckErrorMessage: "Параметр 'MacroErrors' установлен в '%0%', что может привести к неполной обработке части страниц или всех страниц сайта при наличии ошибок в макросах. Устранить это можно путем установки значения в '%1%'.",
		...
		compilationDebugCheckErrorMessage: "Режим компиляции с отладкой сейчас включен. Рекомендуется выключить перед размещением сайта в сети.",
		clickJackingCheckHeaderFound: "Был обнаружен заголовок или мета-тэг <strong>X-Frame-Options</strong>, использующийся для управления возможностью помещать сайт в IFRAME на другом сайте.",
		clickJackingCheckHeaderNotFound: "Заголовок или мета-тэг <strong>X-Frame-Options</strong>, использующийся для управления возможностью помещать сайт в IFRAME на другом сайте, не обнаружен.",
		noSniffCheckHeaderFound: "Заголовок или мета-тэг <strong>X-Content-Type-Options</strong>, использующиеся для защиты от MIME-уязвимостей, обнаружены.",
		noSniffCheckHeaderNotFound: "Заголовок или мета-тэг <strong>X-Content-Type-Options</strong>, использующиеся для защиты от MIME-уязвимостей, не найдены.",
		hSTSCheckHeaderFound: "Заголовок <strong>Strict-Transport-Security</strong>, известный также как HSTS-header, обнаружен.",
		...
		excessiveHeadersFound: "Обнаружены следующие заголовки, позволяющие выяснить базовую технологию сайта: <strong>%0%</strong>.",
		excessiveHeadersNotFound: "Заголовки, позволяющие выяснить базовую технологию сайта, не обнаружены.",
		smtpMailSettingsConnectionSuccess: "Параметры отправки электронной почты (SMTP) настроены корректно, сервис работает как ожидается.",
		notificationEmailsCheckSuccessMessage: "Адрес для отправки уведомлений установлен в <strong>%0%</strong>.",
		notificationEmailsCheckErrorMessage: "Адрес для отправки уведомлений все еще установлен в значение по-умолчанию <strong>%0%</strong>.",
		scheduledHealthCheckEmailBody: "<html><body><p>Зафиксированы следующие результаты автоматической проверки состояния Umbraco по расписанию, запущенной на %0% в %1%:</p>%2%</body></html>",
	...
	installer: {
		...
		databaseInstallDone: "Ваша База данных сконфигурирована для работы Umbraco %0%. Нажмите кнопку <strong>Далее</strong> для продолжения.",
		databaseText: "Для завершения данного шага Вам нужно располагать некоторой информацией о Вашем сервере базы данных\n\t\t(строка подключения \"connection string\")<br />\n\t\tПожалуйста, свяжитесь с Вашим хостинг-провайдером, если есть необходимость, а если устанавливаете на локальную рабочую станцию или сервер, то получите информацию у Вашего системного администратора.",
		...
		databaseUpgradeDone: "Ваша база данных успешно обновлена до последней версии %0%.<br/>Нажмите <strong>Далее</strong> для продолжения. ",
		databaseUpToDate: "Указанная Вами база данных находится в актуальном состоянии. Нажмите кнопку <strong>Далее</strong> для продолжения работы мастера настроек",
		defaultUserChangePass: "<strong>Пароль пользователя по-умолчанию необходимо сменить!</strong>",
		defaultUserDisabled: "<strong>Пользователь по-умолчанию заблокирован или не имеет доступа к Umbraco!</strong></p><p>Не будет предпринято никаких дальнейших действий. Нажмите кнопку <strong>Далее</strong> для продолжения.",
		defaultUserPassChanged: "<strong>Пароль пользователя по-умолчанию успешно изменен в процессе установки!</strong></p><p>Нет надобности в каких-либо дальнейших действиях. Нажмите кнопку <strong>Далее</strong> для продолжения.",
		...
		permissionsAffectedFoldersText: "Вам следует установить разрешения для учетной записи ASP.NET на модификацию следующих файлов и папок",
		permissionsAlmostPerfect: "<strong>Установки разрешений в Вашей системе почти полностью отвечают требованиям Umbraco!</strong>\n\t\t<br /><br />Вы имеете возможность запускать Umbraco без проблем, однако не сможете воспользоваться такой сильной стороной системы Umbraco как установка дополнительных пакетов расширений и дополнений.",
		...
		permissionsHowtoResolveText: "Пожалуйста, посмотрите наш <strong>видео-материал</strong>, посвященный установке разрешений для файлов и папок в Umbraco или прочтите текстовую версию документа.",
		permissionsMaybeAnIssue: "<strong>Установки разрешений в Вашей файловой системе могут быть неверными!</strong>\n\t\t<br /><br />Вы имеете возможность запускать Umbraco без проблем,\n\t\tоднако не сможете воспользоваться такой сильной стороной системы Umbraco как установка дополнительных пакетов расширений и дополнений.",
		permissionsNotReady: "<strong>Установки разрешений в Вашей файловой системе не подходят для работы Umbraco!</strong>\n\t\t<br /><br />Если Вы хотите продолжить работу с Umbraco,\n\t\tВам необходимо скорректировать установки разрешений.",
		permissionsPerfect: "<strong>Установки разрешений в Вашей системе идеальны!</strong>\n\t\t<br /><br />Вы имеете возможность работать с Umbraco в полном объеме включая установку дополнительных пакетов расширений и дополнений!",
		permissionsResolveFolderIssues: "Решение проблемы с папками",
		permissionsResolveFolderIssuesLink: "Воспользуйтесь этой ссылкой для получения более подробной информации о проблемах создания папок от имени учетной записи ASP.NET",
	...
	login: {
		...
		requestPasswordResetConfirmation: "Будет выслано письмо с инструкциями по сбросу пароля на указанный email-адрес, если он совпадает с адресом пользователя",
	...
	moveOrCopy: {
		...
		notValid: "Данное действие не может быть осуществлено, так как Вы не имеете достаточных прав для совершения действий над одним или более дочерними документами.",
	...
	packager: {
		...
		noPackagesDescription: "Вы пока еще не устанавливали ни одного пакета. Вы можете установить как локальный пакет, выбрав файл на Вашем компьютере, так и пакет из репозитория, нажав на значок <strong>'Packages'</strong> наверху справа",
		packageSearch: "Поиск по пакетам",
		packageSearchResults: "Результаты поиска по",
		packageNoResults: "Ничего не найдено по запросу",
		packageNoResultsDescription: "Пожалуйста, повторите поиск, уточнив запрос, или воспользуйтесь просмотром по категориям",
		...
		packageCompatibilityDescription: "Этот пакет совместим со следующими версиями Umbraco, по сообщениям участников сообщества. Полная совместимость не гарантируется для версий со значением ниже 100%",
		...
		packageUninstallText: "Сейчас Вы можете снять отметки с тех опций пакета, которые НЕ хотите удалять. После нажатия кнопки \"Подтверждение деинсталляции\" все отмеченные опции будут удалены.<br />\n\t\t<span style=\"color: Red; font-weight: bold;\">Обратите внимание:</span> все документы, медиа-файлы и другой контент, зависящий от этого пакета, перестанет нормально работать, что может привести к нестабильному поведению системы,\n\t\tпоэтому удаляйте пакеты очень осторожно. При наличии сомнений, свяжитесь с автором пакета.",
		packageVersion: "Версия пакета",
	},
	paste: {
		doNothing: "Вставить, полностью сохранив форматирование (не рекомендуется)",
		errorMessage: "Текст, который Вы пытаетесь вставить, содержит специальные символы и/или элементы форматирования. Это возможно при вставке текста, скопированного из Microsoft Word. Система может удалить эти элементы автоматически, чтобы сделать вставляемый текст более пригодным для веб-публикации.",
	...
	publicAccess: {
		...
		paAdvancedHelp: "Применяйте, если желаете контролировать доступ к документу на основе ролевой модели безопасности,<br /> с использованием групп участников Umbraco.",
		paAdvancedNoGroups: "Вам необходимо создать хотя бы одну группу участников для применения ролевой модели безопасности.",
		paErrorPage: "Страница сообщения об ошибке",
		paErrorPageHelp: "Используется в случае, когда пользователь авторизован в системе, но не имеет доступа к документу.",
		...
		paSimpleHelp: "Применяйте, если хотите установить самый простой способ доступа к документу - явно указанные имя пользователя и пароль",
	},
	publish: {
	...
	redirectUrls: {
		...
		noRedirectsDescription: "Если опубликованный документ переименовывается или меняет свое расположение в дереве, а следовательно, меняется адрес (URL), автоматически создается перенаправление на новое местоположение этого документа.",
	...
	rollback: {
		diffHelp: "Здесь показаны различия между новейшей версией документа и выбранной Вами версией.<br /><del>Красным</del> отмечен текст, которого уже нет в последней версии, <ins>зеленым</ins> - текст, который добавлен",
	...
	settings: {
		...
		importDocumentTypeHelp: "Чтобы импортировать тип документа, найдите файл \".udt\" на своем компьютере, нажав на кнопку \"Обзор\", затем нажмите \"Импортировать\" (на следующем экране будет запрошено подтверждение для этой операции).",
		newtabname: "Заголовок новой вкладки",
		nodetype: "Тип узла (документа)",
		noPropertiesDefinedOnTab: "Для данной вкладки не определены свойства. Кликните по ссылке \"Click here to add a new property\" сверху, чтобы создать новое свойство.",
		objecttype: "Тип",
		script: "Скрипт",
	...
	sort: {
		...
		sortHelp: "Перетаскивайте элементы на нужное место вверх или вниз для определения необходимого Вам порядка сортировки. Также можно использовать заголовки столбцов, чтобы отсортировать все элементы сразу.",
	...
	speechBubbles: {
		...
		validationFailedMessage: "Ошибки, найденные при проверке значений, должны быть исправлены, чтобы было возможно сохранить документ",
	...
	template: {
		...
		insertDictionaryItemDesc: "Статья словаря - это контейнер для части текста, переводимой на разные языки, это позволяет упростить создание многоязычных сайтов.",
		insertMacro: "Макрос",
		...
		insertPageFieldDesc: "Отображает значение указанного поля данных текущей страницы,\n      с возможностью указать альтернативные поля и/или подстановку константы.\n    ",
	...
	translation: {
		...
		noTranslators: "Пользователей-переводчиков не обнаружено. Пожалуйста, создайте пользователя с ролью переводчика, перед тем как отсылать содержимое на перевод",
		...
		translationDoneHelp: "Вы можете просмотреть документы, переведенные Вами, кликнув на ссылке ниже. Если будет найден оригинал документа, Вы увидите его и переведенный вариант в режиме сравнения.",
	...
	update: {
		...
		updateNoServerError: "Во время проверки обновлений произошла ошибка. Пожалуйста, просмотрите журнал трассировки для получения дополнительной информации.",
	},
	user: {
		access: "Доступ",
		accessHelp: "На основании установленных групп и назначенных начальных узлов, пользователь имеет доступ к следующим узлам",
		...
		changePasswordDescription: "Вы можете сменить свой пароль для доступа к административной панели Umbraco, заполнив нижеследующие поля и нажав на кнопку 'Изменить пароль'",
		...
		createUserHelp: "Создавайте новых пользователей, которым нужен доступ к административной панели Umbraco. При создании пользователя для него генерируется новый первичный пароль, который нужно сообщить пользователю.",
		...
		mediastartnodeshelp: "Можно ограничить доступ к медиа-библиотеке (каким-либо ее частям), задав перечень начальных узлов",
		modules: "Разделы",
		...
		startnodeshelp: "Можно ограничить доступ к дереву содержимого (каким-либо его частям), задав перечень начальных узлов",
		userCreated: "Был создан",
		userCreatedSuccessHelp: "Новый первичный пароль успешно сгенерирован. Для входа используйте пароль, приведенный ниже.",
		updateDate: "Время последнего изменения",
		username: "Имя пользователя",
		usergroup: "Группа пользователей",
		userInvited: " был приглашен",
		userInvitedSuccessHelp: "Новому пользователю было отправлено приглашение, в котором содержатся инструкции для входа в панель Umbraco.",
		userinviteWelcomeMessage: "Здравствуйте и добро пожаловать в Umbraco! Все будет готово в течении пары минут, нам лишь нужно задать Ваш пароль для входа.",
