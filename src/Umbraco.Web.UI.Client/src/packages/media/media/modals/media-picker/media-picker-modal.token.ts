...
import type { UmbClipboardPropertyPasteConfig } from '@umbraco-cms/backoffice/clipboard';
...
export type UmbMediaPickerModalData = UmbTreePickerModalData<UmbMediaTreeItemModel> & {
	/**
	 * Shows the clipboard tab, letting the user paste an entry instead of browsing for media. Supply this only
	 * when the opener can act on a picked entry.
	 */
	clipboard?: UmbClipboardPropertyPasteConfig;
};
