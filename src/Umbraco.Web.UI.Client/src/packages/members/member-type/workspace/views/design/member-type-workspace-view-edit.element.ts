...
import type { UmbMemberTypeWorkspaceViewEditTabElement } from './member-type-workspace-view-edit-tab.element.js';
...
@customElement('umb-member-type-workspace-view-edit')
export class UmbMemberTypeWorkspaceViewEditElement extends UmbLitElement implements UmbWorkspaceViewElement {
	...
	#sorter = new UmbSorterController<PropertyTypeContainerModelBaseModel, UUITabElement>(this, {
		...
		identifier: 'member-type-tabs-sorter',
	...
	private _workspaceContext?: UmbMemberTypeWorkspaceContext;

	private _tabsStructureHelper = new UmbContentTypeContainerStructureHelper<UmbMemberTypeDetailModel>(this);
	...
	constructor() {
		...
		this.consumeContext(UMB_WORKSPACE_CONTEXT, (workspaceContext) => {
			this._workspaceContext = workspaceContext as UmbMemberTypeWorkspaceContext;
			this._tabsStructureHelper.setStructureManager((workspaceContext as UmbMemberTypeWorkspaceContext).structure);
			this.observe(
	...
	private _createRoutes() {
		...
		if (this._tabs.length > 0) {
			this._tabs?.forEach((tab) => {
				...
				routes.push({
					...
					component: () => import('./member-type-workspace-view-edit-tab.element.js'),
					setup: (component) => {
		...
		routes.push({
			...
			component: () => import('./member-type-workspace-view-edit-tab.element.js'),
			setup: (component) => {
				(component as UmbMemberTypeWorkspaceViewEditTabElement).noTabName = true;
				(component as UmbMemberTypeWorkspaceViewEditTabElement).ownerTabId = null;
			},
		});
	...
	async #tabNameChanged(event: InputEvent, tab: PropertyTypeContainerModelBaseModel) {
		...
		// Check if it collides with another tab name of this same member-type, if so adjust name:
		if (changedName) {
			newName = changedName;
...
export default UmbMemberTypeWorkspaceViewEditElement;

declare global {
	interface HTMLElementTagNameMap {
		'umb-member-type-workspace-view-edit': UmbMemberTypeWorkspaceViewEditElement;
	}
}
