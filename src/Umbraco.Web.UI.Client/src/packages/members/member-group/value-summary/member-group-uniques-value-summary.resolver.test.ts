...
import { UmbMemberGroupUniquesValueSummaryResolver } from './member-group-uniques-value-summary.resolver.js';
...
@customElement('umb-test-member-group-uniques-value-summary-host')
class UmbTestMemberGroupUniquesValueSummaryHostElement extends UmbControllerHostElementMixin(HTMLElement) {
	constructor() {
		super();
...
describe('UmbMemberGroupUniquesValueSummaryResolver', () => {
	let host: UmbTestMemberGroupUniquesValueSummaryHostElement;
	let resolver: UmbMemberGroupUniquesValueSummaryResolver;
	...
	beforeEach(() => {
		host = new UmbTestMemberGroupUniquesValueSummaryHostElement();
		document.body.appendChild(host);
		resolver = new UmbMemberGroupUniquesValueSummaryResolver(host);
	});
	...
	it('resolves multiple separate member entries to their respective items', async () => {
	...
	it('resolves a member belonging to multiple groups to multiple items', async () => {
		const result = await resolver.resolveValues([[GROUP_ONE_ID, GROUP_TWO_ID]]);
