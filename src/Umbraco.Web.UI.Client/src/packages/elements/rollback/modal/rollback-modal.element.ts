import { UmbElementRollbackRepository } from '../repository/rollback.repository.js';
...
@customElement('umb-element-rollback-modal')
export class UmbElementRollbackModalElement extends UmbModalBaseElement<
	UmbElementRollbackModalData,
	UmbElementRollbackModalValue
> {
	@state()
	private _versions: ElementVersion[] = [];
	...
	#rollbackRepository = new UmbElementRollbackRepository(this);
	...
	#currentElement: UmbElementDetailModel | undefined;
	...
	constructor() {
		...
		this.consumeContext(UMB_ENTITY_CONTEXT, async (instance) => {
			if (!instance) return;
			if (instance.getEntityType() !== UMB_ELEMENT_ENTITY_TYPE) {
				throw new Error(`Entity type is not ${UMB_ELEMENT_ENTITY_TYPE}`);
			}

			const unique = instance.getUnique();

			if (!unique) {
				throw new Error('Element unique is not set');
			}

			const { data } = await new UmbElementDetailRepository(this).requestByUnique(unique);
			if (!data) return;

			this.#currentElement = data;
			const itemVariants = this.#currentElement?.variants ?? [];
	...
	async #requestVersions() {
		if (!this.#currentElement?.unique) {
			throw new Error('Element unique is not set');
		}

		const { data } = await this.#rollbackRepository.requestVersionsByElementId(
			this.#currentElement?.unique,
			this._selectedCulture ?? undefined,
		);
		if (!data) return;

		const tempItems: ElementVersion[] = [];
	...
	async #onRollback() {
		...
		const unique = this.#currentElement?.unique;
		const entityType = this.#currentElement?.entityType;

		if (!unique || !entityType) {
			throw new Error('Element unique or entity type is not set');
		}
	...
	async #setDiffs() {
		...
		const currentPropertyValues = this.#currentElement?.values.filter(
		...
		const currentName = this.#currentElement?.variants.find((x) => x.culture === this._selectedCulture)?.name;
...
export default UmbElementRollbackModalElement;

declare global {
	interface HTMLElementTagNameMap {
		'umb-element-rollback-modal': UmbElementRollbackModalElement;
	}
}
