...
import { UmbElementVariantLanguagePickerElement } from '../../../modals/index.js';
import type {
	UmbElementScheduleModalData,
	UmbElementScheduleModalValue,
...
> {
	...
	@state()
	private readonly _selection: Array<UmbElementScheduleSelectionModel> = [];

	@state()
	private _isAllSelected = false;

	@state()
	private _internalValues: Array<UmbElementScheduleSelectionModel> = [];
	...
	#renderItem(option: UmbElementVariantOptionModel) {
		...
		return html`
			<uui-menu-item
				...
				${UmbElementVariantLanguagePickerElement.renderLabel(option)}
			</uui-menu-item>
	...
	#renderPublishDateInput(option: UmbElementVariantOptionModel, fromDate: string | null, toDate: string | null) {
		return html`
...
export default UmbElementScheduleModalElement;

declare global {
	interface HTMLElementTagNameMap {
		'umb-element-schedule-modal': UmbElementScheduleModalElement;
	}
}
