...
export class UmbElementHistoryWorkspaceInfoAppElement extends UmbLitElement {
	#allowedActions = new Set(['Umb.EntityAction.Element.Rollback']);

	#auditLogRepository = new UmbElementAuditLogRepository(this);
	...
	@state()
	private _items: Array<UmbElementAuditLogModel> = [];
	...
	async #requestAuditLogs() {
		...
		if (!unique) throw new Error('Element unique is required');
		...
		return html`
			<umb-history-list>
				${repeat(
					...
					(item) => item.timestamp,
					(item) => {
						const { text, style } = getElementHistoryTagStyleAndText(item.logType);
...
export default UmbElementHistoryWorkspaceInfoAppElement;

declare global {
	interface HTMLElementTagNameMap {
		'umb-element-history-workspace-info-app': UmbElementHistoryWorkspaceInfoAppElement;
	}
}
