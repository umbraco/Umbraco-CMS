import { UmbracoSearchExamineProviderService } from './api/index.js';
...
export class UmbSearchExamineProviderRepository extends UmbRepositoryBase {
	...
	async requestSearchDocument(documentKey: string | undefined, indexAlias: string | undefined) {
		...
		const { data, error } = await tryExecute(
			this,
			UmbracoSearchExamineProviderService.getDocument({
		...
		return { data: data as UmbExamineDocumentModel | undefined, error };
	}
}
