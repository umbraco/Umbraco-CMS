...
export class UmbSearchExamineShowFieldsModal extends UmbModalBaseElement<UmbExamineShowFieldsModalData> {
	...
	async #requestSearchDocumentFields() {
		...
		const hasPreferred = preferredCulture && this._cultureDocuments.some((d) => d.culture === preferredCulture);
		this._activeCulture = hasPreferred ? preferredCulture : this._cultureDocuments[0]?.culture;
		this._isLoading = false;
	}

	#mapCultureDocument(doc: UmbExamineIndexDocumentModel): CultureDocument {
		...
		return this._cultureDocuments.map(
			(doc) => html`
				<umb-search-examine-document-fields
					.fields=${doc.fields}
					?hidden=${this.#hasMultipleCultures &&
					doc.culture !== this._activeCulture}></umb-search-examine-document-fields>
			`,
		);
	}
			...
			<umb-body-layout headline=${this.localize.term('searchExamine_headline')}>
				...
				<uui-scroll-container id="field-viewer">
					${when(
						...
						() => (this._error ? html`<div class="error-message">${this._error}</div>` : this.#renderDocuments()),
					)}
