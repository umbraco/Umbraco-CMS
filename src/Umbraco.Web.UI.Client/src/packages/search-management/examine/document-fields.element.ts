...
export class UmbSearchExamineDocumentFieldsElement extends UmbLitElement {
	...
	get #filteredAndSortedFields(): Array<UmbExamineFieldModel> {
		...
		return this.fields
			.filter(
				(field) =>
					field.name.toLowerCase().includes(query) || field.values.some((v) => v.toLowerCase().includes(query)),
			)
	...
	#renderField(field: UmbExamineFieldModel) {
