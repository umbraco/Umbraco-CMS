import { UmbSearchQueryRepository } from '../query/search-query.repository.js';
...
import type { UUIInputElement, UUIPaginationElement, UUISelectElement } from '@umbraco-cms/backoffice/external/uui';
...
export class UmbSearchIndexSearchBoxElement extends UmbLitElement {
	...
	private _tableColumns: Array<UmbTableColumn> = [
		{
			name: this.localize.term('searchManagement_tableColumnName'),
			alias: 'name',
		},
		{
			name: this.localize.term('searchManagement_tableColumnEntityType'),
			alias: 'entityType',
		},
		{
			name: '',
		...
		return html`
			<uui-box headline=${this.localize.term('searchManagement_searchBox')}>
				<div
					...
					role="search"
					aria-label=${this.localize.term('searchManagement_searchFormLabel', this._indexAlias ?? '')}
					...
					<div class="search-input-row">
						<uui-input
							...
							placeholder=${this.localize.term('searchManagement_searchPlaceholder')}
							label=${this.localize.term('searchManagement_searchInputLabel')}
							aria-label=${this.localize.term('searchManagement_searchInputAriaLabel', this._indexAlias ?? '')}
						...
						${when(
							...
							() => html`
								<uui-select
									label=${this.localize.term('searchManagement_cultureSelectLabel')}
									.options=${this._languages.map((lang) => ({
										name: lang.name,
										value: lang.unique,
										selected: lang.unique === this._selectedCulture,
									}))}
									@change=${this.#handleCultureChange}></uui-select>
							`,
						)}
						<uui-button
							look="primary"
							...
							label=${this.localize.term('searchManagement_searchButtonAriaLabel')}>
							<umb-localize key="searchManagement_searchButton">Search</umb-localize>
						</uui-button>
					</div>
					...
					${when(
						this._isSearching,
						() => html`
							<div role="status" aria-label=${this.localize.term('searchManagement_loading')}>
					...
					${when(
						this._error,
						() => html` <div class="error-message" role="alert" aria-live="assertive">${this._error}</div> `,
					)}
					${this.#renderResults()}
				</div>
			</uui-box>
	...
	async #handleSearch() {
		...
		this._searchStatusMessage = this.localize.term('searchManagement_searching');
		...
		if (error || !data) {
			...
			this._searchStatusMessage = this.localize.term('searchManagement_searchComplete', data.total);
		}
	...
	#renderResults() {
		...
		return html`
			<div class="results-container" role="region" aria-label=${this.localize.term('searchManagement_resultsRegion')}>
				<div class="results-header" id="results-summary">
					<strong>
						<umb-localize key="searchManagement_resultsCount" .args=${[this._searchResults.total]}>
				...
				<umb-table
					...
					aria-label=${this.localize.term('searchManagement_resultsTable')}>
				</umb-table>
				${this._totalPages > 1
					? html`
							<uui-pagination
								...
								aria-label=${this.localize.term('searchManagement_paginationLabel')}></uui-pagination>
						`
					: nothing}
			</div>
		`;
