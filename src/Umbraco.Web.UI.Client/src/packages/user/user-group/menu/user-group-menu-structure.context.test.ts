...
import { UmbUserGroupMenuStructureWorkspaceContext } from './user-group-menu-structure.context.js';
...
@customElement('umb-test-user-group-menu-structure-host')
class UmbTestUserGroupMenuStructureHostElement extends UmbControllerHostElementMixin(HTMLElement) {
...
describe('UmbUserGroupMenuStructureWorkspaceContext', () => {
	let host: UmbTestUserGroupMenuStructureHostElement;
	let context: UmbUserGroupMenuStructureWorkspaceContext;

	beforeEach(async () => {
		host = await fixture(html`<umb-test-user-group-menu-structure-host></umb-test-user-group-menu-structure-host>`);
		context = new UmbUserGroupMenuStructureWorkspaceContext(host as unknown as UmbControllerHost);
	...
	it('includes only the root item while the entity is new', async () => {
		host.workspaceContext.setEntityType(UMB_USER_GROUP_ENTITY_TYPE);
		host.workspaceContext.setUnique(null);
		host.workspaceContext.setName('New User Group');
