import { manifest as propertyValueEntityReference } from './property-value-entity-reference/manifest.js';
import { manifest as propertyValueResolver } from './property-value-resolver/manifest.js';
import { manifest as schemaManifest } from './Umbraco.RichText.js';
import { manifests as validationManifests } from './validation/manifests.js';
import type { UmbExtensionManifestKind } from '@umbraco-cms/backoffice/extension-registry';

export const manifests: Array<UmbExtensionManifest | UmbExtensionManifestKind> = [
	...validationManifests,
	propertyValueEntityReference,
	propertyValueResolver,
	schemaManifest,
];

export const name = 'Umbraco.Core.Rte';
export const extensions = [
	{
		name: 'RTE Bundle',
		alias: 'Umb.Bundle.Rte',
		type: 'bundle',
		js: {
			manifests,
		},
	},
];
