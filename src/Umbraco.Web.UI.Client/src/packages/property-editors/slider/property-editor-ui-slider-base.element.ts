...
{
	/**
	 * Whether this slider holds a range of two values rather than a single one.
	 */
	protected abstract readonly enableRange: boolean;
	...
	#onChange(event: CustomEvent & { target: UmbInputSliderElement }) {
		...
		this.value = {
			from: handledFrom,
			to: this.enableRange ? undefinedFallback(partialValue.to, this._initVal2) : handledFrom,
		};
