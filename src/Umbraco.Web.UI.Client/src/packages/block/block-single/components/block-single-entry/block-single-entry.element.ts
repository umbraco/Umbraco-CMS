 ...
 */
@customElement('umb-block-single-entry')
export class UmbBlockSingleEntryElement extends UmbLitElement implements UmbPropertyEditorUiElement {
	//
	...
	@state()
	private _blockViewProps: UmbBlockEditorCustomViewProperties<UmbBlockSingleLayoutModel> = {
	...
	#extensionSlotFilterMethod = (manifest: ManifestBlockEditorCustomView) => {
		...
		if (manifest.forBlockEditor && !stringOrStringArrayContains(manifest.forBlockEditor, UMB_BLOCK_SINGLE)) {
			return false;
		}
		return true;
	};
...
export default UmbBlockSingleEntryElement;

declare global {
	interface HTMLElementTagNameMap {
		'umb-block-single-entry': UmbBlockSingleEntryElement;
	}
}
