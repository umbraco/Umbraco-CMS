...
> {
    ...
    #renderRenderChildTemplate() {
        return html`<uui-card
            ...
            ${this._pickedSection === TemplatingSectionType.renderChildTemplate
                /* CHANGED: Added slot="badge" and look="primary" */
                ? html`<uui-badge slot="badge" look="primary" color="positive"><uui-icon name="icon-check"></uui-icon></uui-badge>`
                : ''}
    ...
    static override styles = [
        ...
        css`
            ...
            /* CHANGED: Added pointer-events none to text so it doesn't break the card hover state */
            uui-card h3,
            uui-card p {
                pointer-events: none;
            }
