...
describe('UmbPropertyGuardManager', () => {
	...
	describe('Fallback', () => {
		it('isPermittedForProperty reacts to late fallback updates when no rules match', () => {
			const emitted: boolean[] = [];
			const subscription = manager.isPermittedForProperty(propA).subscribe((value) => emitted.push(value));
		...
		it('isPermittedForProperty is stable when a matching rule determines the result', () => {
			manager.addRule(rulePropB);
			const emitted: boolean[] = [];
			const subscription = manager.isPermittedForProperty(propB).subscribe((value) => emitted.push(value));
		...
		it('isPermittedForProperty uses late fallback updates for properties without a matching rule', () => {
			manager.addRule(rulePropB);
			const emitted: boolean[] = [];
			const subscription = manager.isPermittedForProperty(propA).subscribe((value) => emitted.push(value));
		...
		it('getIsPermittedForProperty reflects the current fallback when no rules match', () => {
			expect(manager.getIsPermittedForProperty(propA)).to.equal(false);
			manager.fallbackToPermitted();
			expect(manager.getIsPermittedForProperty(propA)).to.equal(true);
			manager.fallbackToNotPermitted();
			expect(manager.getIsPermittedForProperty(propA)).to.equal(false);
		});
	});
