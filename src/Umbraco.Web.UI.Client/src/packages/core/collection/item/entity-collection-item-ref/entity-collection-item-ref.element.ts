...
import type { ManifestEntityCollectionItemRef } from './entity-collection-item-ref.extension.js';
...
export class UmbEntityCollectionItemRefElement extends UmbLitElement {
	...
	public set item(value: UmbCollectionItemModel | undefined) {
		...
		this.#pathAddendum.setAddendum('collection-item-ref/' + value.entityType + '/' + value.unique);
	...
	protected override firstUpdated(_changedProperties: PropertyValueMap<any> | Map<PropertyKey, unknown>): void {
		...
		this.setAttribute(UMB_MARK_ATTRIBUTE_NAME, 'entity-collection-item-ref');
	}
	...
	#createController(entityType: string) {
		...
		this.#extensionsController = new UmbExtensionsElementInitializer(
			...
			(manifest: ManifestEntityCollectionItemRef) => manifest.forEntityTypes.includes(entityType),
			(extensionControllers) => {
				this._component?.remove();
				const component =
					extensionControllers[0]?.component || document.createElement('umb-default-collection-item-ref');
