...
> extends UmbControllerBase {
    ...
    #isSectionWithPartialLoad: boolean = false;
    #requestMaxRetries = 2;
    ...
    constructor(host: UmbControllerHost) {
        ...
        this.consumeContext(UMB_SECTION_CONTEXT, (sectionContext) => {
            ...
            this.observe(
                ...
                (pathname) => {
                    if (!pathname) return;
                    this.#isSectionWithPartialLoad = pathname === 'content' || pathname === 'media';
                    this.#updateTakeSize();
                },
                'observeSectionForTakeSize',
            );
        });
    }
    ...
    #updateTakeSize() {
        ...
        if (this.#isSectionWithPartialLoad && !isRoot) {
            // Apply the 5 item limit ONLY to nested children in Content/Media
            this.setTakeSize(5);
        } else {
            // Load everything for Root items, or items in other sections
            this.setTakeSize(100000);
        }
    }
    ...
    #onPageChange = () => {
        ...
        if (this.#isSectionWithPartialLoad) {
