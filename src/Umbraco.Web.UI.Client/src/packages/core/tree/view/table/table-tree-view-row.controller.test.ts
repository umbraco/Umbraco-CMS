...
import { UMB_TREE_ITEM_API_CONTEXT } from '../../tree-item/tree-item.context.token.js';
...
describe('UmbTableTreeViewRowController', () => {
	...
	describe('context provision', () => {
		it('provides the tree item api context on the row element', async () => {
			...
			const api = await child.getContext(UMB_TREE_ITEM_API_CONTEXT);
	...
	describe('destroy', () => {
		it('removes the provided contexts', async () => {
			controller = new UmbTableTreeViewRowController(host, treeItem.entityType, treeItem.unique, treeItem);
			expect(await resolvesWithin(child.getContext(UMB_TREE_ITEM_API_CONTEXT), 100)).to.be.true;

			controller.destroy();
			controller = undefined;

			expect(await resolvesWithin(child.getContext(UMB_TREE_ITEM_API_CONTEXT), 50)).to.be.false;
		});
