...
describe('UmbPropertyClipboardManager', () => {
	...
	describe('readMultiple', () => {
		it('returns an empty array for an empty uniques list', async () => {
			provideClipboardContext();
			providePropertyContext();
			provideDatasetContext();
			const manager = await createManager();

			expect(await manager.readMultiple([])).to.deep.equal([]);
		});

		it('returns an empty array when the clipboard context is not available', async () => {
			providePropertyContext();
			provideDatasetContext();
			const manager = await createManager();

			expect(await manager.readMultiple(['some-unique'])).to.deep.equal([]);
		});

		it('reads entries and returns the translated property values', async () => {
			provideClipboardContext();
			providePropertyContext();
			provideDatasetContext();
			const manager = await createManager();
			...
			const values = await manager.readMultiple<string>([entry.unique]);

			expect(values).to.deep.equal(['hello property value']);
		});

		it('returns an empty array when the property editor UI alias cannot be resolved', async () => {
			provideClipboardContext();
			providePropertyContext({ alias: undefined });
			provideDatasetContext();
			const manager = await createManager();
		...
		it('returns an empty array when the clipboard read fails to resolve any entry', async () => {
			provideClipboardContext();
			providePropertyContext();
			provideDatasetContext();
			const manager = await createManager();

			// No entry with this unique exists, so the underlying read throws and the manager swallows it.
			expect(await manager.readMultiple(['non-existent-unique'])).to.deep.equal([]);
		});
	});
});
