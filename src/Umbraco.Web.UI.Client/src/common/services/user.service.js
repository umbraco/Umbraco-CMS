angular.module('umbraco.services')
  .factory('userService', function ($rootScope, eventsService, $q, $location, $window, requestRetryQueue, authResource, emailMarketingResource, $timeout, angularHelper, usersResource, contentEditingHelper) {
    ...
    */
    ...
    return {
      ...
      /** Tries to change the user password and invokes the ContentEditingHelper on errors */
      changePassword: (changePasswordModel, user) => {
        ...
        if (changePasswordModel.value) {
          ...
          changePasswordModel.value.reset = (!changePasswordModel.value.oldPassword && !user.isCurrentUser) || changePasswordModel.config.allowManuallyChangingPassword;
        }

        // since we don't send the entire user model, the id is required
        changePasswordModel.value.id = user.id;

        return usersResource.changePassword(changePasswordModel.value)
          .then(() => {
            changePasswordModel.isChanging = false;
            changePasswordModel.value = {};

            //the user has a password if they are not states: Invited, NoCredentials
            changePasswordModel.config.hasPassword = user.userState !== "Invited" && user.userState !== "Inactive";
