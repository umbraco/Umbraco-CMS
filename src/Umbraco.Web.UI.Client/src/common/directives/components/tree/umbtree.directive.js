    ...
    controller: function ($scope, $element) {
      ...
      function emitEvent(eventName, args) {
        if (
          ...
          Utilities.isArray(registeredCallbacks[eventName])
        ) {
          // call it
      ...
      function hasTree(treeAlias) {
        ...
        var foundTree = _.find(treeRoots, function (node) {
          return (
            node.metaData.treeAlias.toUpperCase() === treeAlias.toUpperCase()
          );
        });
      ...
      function loadTree() {
        if ($scope.section) {
          ...
          return treeService.getTree(args).then(
            function (data) {
      ...
      $scope.selectEnabledNodeClass = (node) =>
        node && node.selected
          ? "icon sprTree icon-check green temporary"
          : "-hidden";

      /* helper to force reloading children of a tree node */
      $scope.loadChildren = (node, forceReload) =>
        loadChildren(node, forceReload);
      ...
      $q.when($scope.onInit(), function (args) {
        ...
        localizationService
          .localize("visuallyHiddenTexts_openContextNode")
          .then((value) => {
