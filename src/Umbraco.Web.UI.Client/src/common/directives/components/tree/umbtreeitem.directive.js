    ...
    .directive('umbTreeItem', function ($compile, $http, $templateCache, $interpolate, $log, $location, $rootScope, $window, treeService, $timeout, localizationService) {
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                // Helper function for swap camel case to a sentence case string
                ...
                function setupNodeDom(node, tree) {
                    ...
                    // Get the first anchor element in the template
                    var a1 = element.find("a:first");
                    a1.text(node.name);
                    ...
                    if (node.nodeType === "content") {
                        ...
                        localizationService.localize("content_wcag_label").then(function (value) {
                            wcagLabel = value;
                        });

                        var a1c = node.metaData.contentType !== undefined ? node.metaData.contentType : node.name;

                        

                        wcagLabel = wcagLabel + ": " + a1c

                        a1.attr("title", wcagLabel);

                        var wcag1 = element.find("span:first");

                        wcag1.addClass("hide-text");

                        wcag1.text(wcagLabel);
                    }
