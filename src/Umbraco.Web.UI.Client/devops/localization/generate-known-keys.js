 ...
 * each `group: { key: value }` pair to a single `group_key` property, and emits two files:
 *
 * - `src/libs/localization-api/known-keys.types.generated.ts` — the `UmbKnownLocalizationSet`
 *   interface and `UmbKnownLocalizationKey` type, declared globally. The interface preserves
 *   whether each entry is a plain string or a function — so `localize.term()` keeps its
 *   argument-type inference via `LocalizationArgsOf<LocalizationSetType, K>` in
 *   `localization.controller.ts`.
 * - `src/libs/localization-api/known-keys.const.generated.ts` — the `KNOWN_LOCALIZATION_KEYS`
 *   runtime array, consumed by `known-keys.test.ts` and the `no-unknown-localization-key`
 *   ESLint rule.
 *
...
const outputDir = path.join(projectRoot, 'src/libs/localization-api');
const typesOutputPath = path.join(outputDir, 'known-keys.types.generated.ts');
const constOutputPath = path.join(outputDir, 'known-keys.const.generated.ts');

const generatedNotice = `// AUTO-GENERATED — DO NOT EDIT
//
...
const typesHeader = `${generatedNotice}
//
...
const constHeader = `${generatedNotice}
`;
...
async function main() {
	...
	const prettierConfig = (await resolveConfig(typesOutputPath)) ?? {};
	const formattedTypes = await format(typesBody, { ...prettierConfig, parser: 'typescript' });
	const formattedConst = await format(constBody, { ...prettierConfig, parser: 'typescript' });

	let changed = false;
	for (const [outputPath, formatted] of [
		[typesOutputPath, formattedTypes],
		[constOutputPath, formattedConst],
	]) {
