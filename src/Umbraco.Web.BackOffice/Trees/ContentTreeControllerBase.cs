    ...
    public abstract class ContentTreeControllerBase : TreeController, ITreeNodeController
    {
        ...
        private void GetUserStartNodes(out int[] startNodeIds, out string[] startNodePaths)
        {
            switch (RecycleBinId)
            {
                case Constants.System.RecycleBinMedia:
                    startNodeIds = _webSecurityAccessor.WebSecurity.CurrentUser.CalculateMediaStartNodeIds(_entityService);
                    startNodePaths = _webSecurityAccessor.WebSecurity.CurrentUser.GetMediaStartNodePaths(_entityService);
                    break;
                case Constants.System.RecycleBinContent:
                    startNodeIds = _webSecurityAccessor.WebSecurity.CurrentUser.CalculateContentStartNodeIds(_entityService);
                    startNodePaths = _webSecurityAccessor.WebSecurity.CurrentUser.GetContentStartNodePaths(_entityService);
                    break;
                default:
        ...
        protected virtual TreeNodeCollection PerformGetTreeNodes(string id, FormCollection queryStrings)
        {
            ...
            if (string.IsNullOrEmpty(startNodeId) == false && startNodeId != "undefined" && startNodeId != rootIdString)
            {
                ...
                {
                    _logger.Warn<ContentTreeControllerBase>("User {Username} does not have access to node with id {Id}", _webSecurityAccessor.WebSecurity.CurrentUser.Username, id);
                    return nodes;
                }
        ...
        protected bool HasPathAccess(IUmbracoEntity entity, FormCollection queryStrings)
        {
            ...
            return RecycleBinId == Constants.System.RecycleBinContent
                ? _webSecurityAccessor.WebSecurity.CurrentUser.HasContentPathAccess(entity, _entityService)
                : _webSecurityAccessor.WebSecurity.CurrentUser.HasMediaPathAccess(entity, _entityService);
        }
        ...
        protected sealed override MenuItemCollection GetMenuForNode(string id, FormCollection queryStrings)
        {
            if (RecycleBinId.ToInvariantString() == id)
            {
                ...
                {
                    var perms = _webSecurityAccessor.WebSecurity.CurrentUser.GetPermissions(Constants.System.RecycleBinContentString, _userService);
        ...
        {
            var permissionsForPath = _userService.GetPermissionsForPath(_webSecurityAccessor.WebSecurity.CurrentUser, dd.Path).GetAllPermissions();
