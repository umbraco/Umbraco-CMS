    ...
    public class PackageInstallController : UmbracoAuthorizedJsonController
    {
        ...
        public IActionResult Uninstall(int packageId)
        {
            try
            {
                ...
                var summary = _packagingService.UninstallPackage(package.Name, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
                ...
                {
                    //remove from the xml
                    _packagingService.DeleteInstalledPackage(installed.Id, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<LocalPackageInstallModel> Fetch(string packageGuid)
        {
            ...
            if (System.IO.File.Exists(Path.Combine(_hostingEnvironment.MapPathContentRoot(Constants.SystemDirectories.Packages), fileName)) == false)
            {
                var packageFile = await _packagingService.FetchPackageFileAsync(
                    ...
                    _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
        ...
        public PackageInstallModel InstallFiles(PackageInstallModel model)
        {
            ...
            var installedFiles = _packagingService.InstallCompiledPackageFiles(definition, zipFile, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
        ...
        public PackageInstallModel InstallData(PackageInstallModel model)
        {
            ...
            var zipFile = new FileInfo(definition.PackagePath);
            var installSummary = _packagingService.InstallCompiledPackageData(definition, zipFile, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
