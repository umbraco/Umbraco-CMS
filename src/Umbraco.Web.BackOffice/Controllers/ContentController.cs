...
namespace Umbraco.Web.Editors
{
    ...
    public class ContentController : ContentControllerBase
    {
        ...
        {
            var emptyContent = _contentService.Create("", parentId, contentType.Alias, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
        ...
        public ActionResult<SimpleNotificationModel> CreateBlueprintFromContent([FromQuery]int contentId, [FromQuery]string name)
        {
            ...
            var blueprint = _contentService.CreateContentFromBlueprint(content, name, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));

            _contentService.SaveBlueprint(blueprint, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
         ...
         public ContentItemDisplay PostSaveBlueprint([ModelBinder(typeof(BlueprintItemBinder))] ContentItemSave contentItem)
         {
             var contentItemDisplay = PostSaveInternal(contentItem,
                 content =>
                 {
                     ...
                     _contentService.SaveBlueprint(contentItem.PersistedContent, _webSecurityAccessor.WebSecurity.CurrentUser.Id);
        ...
        public ContentItemDisplay PostSave([ModelBinder(typeof(ContentItemBinder))] ContentItemSave contentItem)
        {
            var contentItemDisplay = PostSaveInternal(
                contentItem,
                content => _contentService.Save(contentItem.PersistedContent, _webSecurityAccessor.WebSecurity.CurrentUser.Id),
                MapToDisplay);
        ...
        private ContentItemDisplay PostSaveInternal(ContentItemSave contentItem, Func<IContent, OperationResult> saveMethod, Func<IContent, ContentItemDisplay> mapToDisplay)
        {
            ...
            switch (contentItem.Action)
            {
                ...
                case ContentSaveAction.SendPublish:
                case ContentSaveAction.SendPublishNew:
                    var sendResult = _contentService.SendToPublication(contentItem.PersistedContent, _webSecurityAccessor.WebSecurity.CurrentUser.Id);
                ...
                foreach (var c in descendants)
                {
                    ...
                    if (denied.Any(x => c.Path.StartsWith($"{x.Path},"))
                        || (ContentPermissionsHelper.CheckPermissions(c,
                            _webSecurityAccessor.WebSecurity.CurrentUser, _userService, _entityService,
                ...
                out bool wasCancelled, out string[] successfulCultures)
        {
            if (!contentItem.PersistedContent.ContentType.VariesByCulture())
            {
                ...
                var publishStatus = _contentService.SaveAndPublishBranch(contentItem.PersistedContent, force, userId: _webSecurityAccessor.WebSecurity.CurrentUser.Id);
            ...
            if (canPublish)
            {
                ...
                var publishStatus = _contentService.SaveAndPublishBranch(contentItem.PersistedContent, force, culturesToPublish, _webSecurityAccessor.WebSecurity.CurrentUser.Id);
                // TODO: Deal with multiple cancellations
                wasCancelled = publishStatus.Any(x => x.Result == PublishResultType.FailedPublishCancelledByEvent);
            ...
            {
                //can only save
                var saveResult = _contentService.Save(contentItem.PersistedContent, _webSecurityAccessor.WebSecurity.CurrentUser.Id);
        ...
        private PublishResult PublishInternal(ContentItemSave contentItem, string defaultCulture, string cultureForInvariantErrors, out bool wasCancelled, out string[] successfulCultures)
        {
            ...
            {
                //its invariant, proceed normally
                var publishStatus = _contentService.SaveAndPublish(contentItem.PersistedContent, userId: _webSecurityAccessor.WebSecurity.CurrentUser.Id);
            ...
            {
                //proceed to publish if all validation still succeeds
                var publishStatus = _contentService.SaveAndPublish(contentItem.PersistedContent, culturesToPublish, _webSecurityAccessor.WebSecurity.CurrentUser.Id);
                wasCancelled = publishStatus.Result == PublishResultType.FailedPublishCancelledByEvent;
        ...
        public IActionResult PostPublishById(int id)
        {
            ...
            var publishResult = _contentService.SaveAndPublish(foundContent, userId: _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
        ...
        public IActionResult DeleteById(int id)
        {
            ...
            {
                var moveResult = _contentService.MoveToRecycleBin(foundContent, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
            ...
            {
                var deleteResult = _contentService.Delete(foundContent, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
        ...
        {
            _contentService.EmptyRecycleBin(_webSecurityAccessor.WebSecurity.GetUserId().ResultOr(Constants.Security.SuperUserId));
        ...
        public IActionResult PostSort(ContentSortOrder sorted)
        {
            ...
            try
            {
                ...
                var sortResult = contentService.Sort(sorted.IdSortOrder, _webSecurityAccessor.WebSecurity.CurrentUser.Id);
        ...
        public IActionResult PostMove(MoveOrCopy move)
        {
            ...
            _contentService.Move(toMove, move.ParentId, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
        ...
        public IActionResult PostCopy(MoveOrCopy copy)
        {
            ...
            var c = _contentService.Copy(toCopy, copy.ParentId, copy.RelateToOriginal, copy.Recursive, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
        ...
        public ContentItemDisplay PostUnpublish(UnpublishContent model)
        {
            ...
            if (model.Cultures.Length == 0 || model.Cultures.Length == languageCount)
            {
                ...
                var unpublishResult = _contentService.Unpublish(foundContent, userId: _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
            ...
            else
            {
                ...
                {
                    var result = _contentService.Unpublish(foundContent, culture: c, userId: _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
                    results[c] = result;
        ...
        public ActionResult<DomainSave> PostSaveLanguageAndDomains(DomainSave model)
        {
            ...
            var permission = _userService.GetPermissions(_webSecurityAccessor.WebSecurity.CurrentUser, node.Path);
        ...
        private ContentItemDisplay MapToDisplay(IContent content)
        {
            ...
            {
                context.Items["CurrentUser"] = _webSecurityAccessor.WebSecurity.CurrentUser;
            });
        ...
        public ActionResult<IEnumerable<NotifySetting>> GetNotificationOptions(int contentId)
        {
            ...
            var userNotifications = _notificationService.GetUserNotifications(_webSecurityAccessor.WebSecurity.CurrentUser, content.Path).ToList();
        ...
        public IActionResult PostNotificationOptions(int contentId, [FromQuery(Name="notifyOptions[]")] string[] notifyOptions)
        {
            ...
            _notificationService.SetNotifications(_webSecurityAccessor.WebSecurity.CurrentUser, content, notifyOptions);
        ...
        {
            var rollbackResult = _contentService.Rollback(contentId, versionId, culture, _webSecurityAccessor.WebSecurity.GetUserId().ResultOr(0));
