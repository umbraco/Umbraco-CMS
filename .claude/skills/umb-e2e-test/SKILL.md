...
- **There is no lint step**, but `npm run audit` covers the mechanical part: it fails at budget 0 on dropped promises, silently-discarded assertions, specs no project runs, raw `page` use in a spec, hardcoded endpoints, and un-annotated skipped tests. Known debt (sleeps, force clicks, substring name locators, untyped builders) is budgeted so it can shrink but not grow — see `CLAUDE.md` §7. **Both gates run in CI** (`build/azure-pipelines.yml`, `Build` stage, job C), so a regression fails the build.
