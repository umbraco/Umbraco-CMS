...
var TinyMCE_AdvancedTheme = {
	...
	getControlHTML : function(button_name) {
		...
		switch (button_name) {
		    case "umbracoformatselector":
		    
				return '<select id="{$editor_id}_umbracoFormatSelect" onmousedown="tinyMCE.themes.advanced._setupCSSClasses(\'{$editor_id}\');" name="{$editor_id}_styleSelect" ' +
				'onfocus="tinyMCE.addSelectAccessibility(event,this,window);" ' +
				'onchange="if (this.options[this.selectedIndex].value != \'\' && this.options[this.selectedIndex].value.indexOf(\'.\') == 0) {' +
				'tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceSetCSSClass\',false,this.options[this.selectedIndex].value.substring(1, this.options[this.selectedIndex].value.length)); }'+
				'else if (this.options[this.selectedIndex].value != \'\') { ' +
				'tinyMCE.execInstanceCommand(\'{$editor_id}\',\'FormatBlock\',false,this.options[this.selectedIndex].value); }' +
				' else {tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceSetCSSClass\',false,\'\'); tinyMCE.execInstanceCommand(\'{$editor_id}\',\'FormatBlock\',false,\'&lt;p&gt;\'); } ' +
			...
			case "styleselect":
				return '<select id="{$editor_id}_styleSelect" onmousedown="tinyMCE.themes.advanced._setupCSSClasses(\'{$editor_id}\');" name="{$editor_id}_styleSelect" onfocus="tinyMCE.addSelectAccessibility(event,this,window);" onchange="alert(this.options[this.selectedIndex].value);tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceSetCSSClass\',false,this.options[this.selectedIndex].value);" class="mceSelectList">{$style_select_options}</select>';
		...
		switch(layoutManager) {
			case "SimpleLayout" : //the default TinyMCE Layout (for backwards compatibility)...
				...
				if (toolbarLocation == "external") {
					...
					// ########################################################################
					// *** umbraco changes ***
					elm.innerHTML = '<table border="0"><tr><td><nobr>'+toolbarHTML+'</nobr></td></tr></table>';

					//bod.appendChild (elm);
					document.getElementById(tinyMCE.getParam("umbraco_toolbar_id", "*")).appendChild(elm);
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection, setup_content) {
		...
		var selectElm = document.getElementById(editor_id + "_umbracoFormatSelect");
