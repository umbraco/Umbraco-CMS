...
- **Switching users**: `UserApiHelper.loginToUser()` signs the current session out first, then signs the new user in — one cookie, one session. Restore the admin with `ApiHelpers.loginToAdminUser()` when the test is done.
- **Reusing the session**: `tests/auth.setup.ts` signs in once and writes `storageState`; every authenticated project depends on that `setup` project and loads the state. Add a new project the same way rather than signing in per spec.
