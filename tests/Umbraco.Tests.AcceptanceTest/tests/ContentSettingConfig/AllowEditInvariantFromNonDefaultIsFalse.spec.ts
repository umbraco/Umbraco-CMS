...
test.skip('can edit variant text property inside invariant block in non-default language when AllowEditInvariantFromNonDefault is false', {annotation: {type: 'issue', description: "This test fails due to the issue: https://github.com/umbraco/Umbraco-CMS/issues/22001"}}, async ({umbracoApi, umbracoUi}) => {
  // Arrange
  const documentTypeId = await umbracoApi.documentType.createDocumentTypeWithVariantAndInvariantBlockLists(
    documentTypeName,
    firstTextName,
