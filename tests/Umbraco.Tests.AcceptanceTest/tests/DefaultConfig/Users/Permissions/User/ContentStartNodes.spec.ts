...
test("can see root start node and children", async ({
  umbracoApi,
  umbracoUi,
}) => {
  // Arrange
...
test.only("see no-access view when deep-linking to restricted document", async ({
  umbracoApi,
  umbracoUi,
}) => {
  // Arrange
...
test("can not see any content when no start nodes specified", async ({
  umbracoApi,
  umbracoUi,
}) => {
  // Arrange
