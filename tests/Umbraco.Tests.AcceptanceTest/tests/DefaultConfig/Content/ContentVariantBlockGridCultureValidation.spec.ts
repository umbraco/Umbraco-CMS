...
const blockGridName = 'TestBlockGrid';
...
test('can publish english variant when danish has no blocks and minimum is required', async ({umbracoApi, umbracoUi}) => {
  ...
  const documentTypeId = await umbracoApi.documentType.createDocumentTypeWithPropertyEditor(documentTypeName, blockGridName, blockGridId, 'Content', true, true);
