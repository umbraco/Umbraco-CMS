...
test('can trigger when content is published', async ({umbracoApi, umbracoUi}) => {
  ...
  // Assert
  const webhookSiteData = await umbracoApi.webhook.getWebhookSiteRequestResponse(webhookSiteToken, contentId);
...
test('can trigger when media is saved', async ({umbracoApi, umbracoUi}) => {
  ...
  // Assert
  const webhookSiteData = await umbracoApi.webhook.getWebhookSiteRequestResponse(webhookSiteToken, mediaId);
...
test('cannot trigger when the webhook is disabled', async ({umbracoApi, umbracoUi}) => {
  ...
  // Assert
  const webhookSiteData = await umbracoApi.webhook.getWebhookSiteRequestResponse(webhookSiteToken, undefined, 7000);
