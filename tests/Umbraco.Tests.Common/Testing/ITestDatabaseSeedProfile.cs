...
///     and creates the snapshot, further fixtures restore from the snapshot instantly.
/// </summary>
public interface ITestDatabaseSeedProfile
{
    /// <summary>
    ///     Gets the unique key identifying this seed profile. Fixtures returning the same key
    ///     share the same database snapshot.
    /// </summary>
    string SeedKey { get; }

    /// <summary>
    ///     Seeds the database. Called once per unique <see cref="SeedKey"/>.
