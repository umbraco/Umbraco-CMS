...
public class NPocoSqlTests : BaseUsingSqlSyntax
{
    ...
    public void ForUpdate()
    {
        ...
        Assert.That(
            sql.SQL.NoCrLf(),
            Is.EqualTo("SELECT * FROM [umbracoUserLogin] WHERE (([umbracoUserLogin].[sessionId] = @0))"));

        sql = sql.ForUpdate();

        Assert.That(
            sql.SQL.NoCrLf(),
            Is.EqualTo("SELECT * FROM [umbracoUserLogin] WITH (UPDLOCK) WHERE (([umbracoUserLogin].[sessionId] = @0))"));
    }
}
