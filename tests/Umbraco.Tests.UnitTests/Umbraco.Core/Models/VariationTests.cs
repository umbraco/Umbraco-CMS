...
public class VariationTests
{
    ...
    public void ContentPublishValuesWithMixedPropertyTypeVariations()
    {
        ...
        Assert.That(
            content.PublishCulture(langFrImpact, DateTime.UtcNow, _propertyEditorCollection),
            Is.True); // succeeds because names are ok (not validating properties here)
        Assert.That(
            propertyValidationService.IsPropertyDataValid(content, out _, langFrImpact),
            Is.False); // fails because prop1 is mandatory
...
#pragma warning disable CS0618 // Type or member is obsolete - IsPropertyValid() will be retained as internal after the obsoletion period.
        ...
        Assert.That(propertyValidationService.IsPropertyValid(prop, PropertyValidationContext.CultureAndSegment(culture, "*")), Is.True);
#pragma warning restore CS0618 // Type or member is obsolete
    }
...
#pragma warning disable CS0618 // Type or member is obsolete - IsPropertyValid() will be retained as internal after the obsoletion period.
        ...
        Assert.That(propertyValidationService.IsPropertyValid(prop, PropertyValidationContext.CultureAndSegment(culture, "*")), Is.False);
#pragma warning restore CS0618 // Type or member is obsolete
    }
