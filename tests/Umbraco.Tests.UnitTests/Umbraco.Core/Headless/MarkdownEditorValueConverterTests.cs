...
public class MarkdownEditorValueConverterTests : PropertyValueConverterTests
{
    ...
    public void MarkdownEditorValueConverter_ConvertsValueToMarkdownString(object inter, string expected)
    {
        ...
        Assert.AreEqual(typeof(string), valueConverter.GetHeadlessPropertyValueType(Mock.Of<IPublishedPropertyType>()));
        var result = valueConverter.ConvertIntermediateToHeadlessObject(Mock.Of<IPublishedElement>(), Mock.Of<IPublishedPropertyType>(), PropertyCacheLevel.Element, inter, false);
