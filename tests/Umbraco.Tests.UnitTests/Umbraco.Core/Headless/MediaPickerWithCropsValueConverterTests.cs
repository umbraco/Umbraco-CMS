...
public class MediaPickerWithCropsValueConverterTests : PropertyValueConverterTests
{
    ...
    public void MediaPickerWithCropsValueConverter_InSingleMode_ConvertsValueToMedia()
    {
        ...
        var valueConverter = new MediaPickerWithCropsValueConverter(PublishedSnapshotAccessor, PublishedUrlProvider, Mock.Of<IPublishedValueFallback>(), serializer, new HeadlessContentNameProvider());
        Assert.AreEqual(typeof(HeadlessMediaWithCrops), valueConverter.GetHeadlessPropertyValueType(publishedPropertyType));
        ...
        var result = valueConverter.ConvertIntermediateToHeadlessObject(Mock.Of<IPublishedElement>(), publishedPropertyType, PropertyCacheLevel.Element, inter, false) as HeadlessMediaWithCrops;
    ...
    public void MediaPickerWithCropsValueConverter_InMultiMode_ConvertsValueToMedias()
    {
        ...
        var valueConverter = new MediaPickerWithCropsValueConverter(PublishedSnapshotAccessor, PublishedUrlProvider, Mock.Of<IPublishedValueFallback>(), serializer, new HeadlessContentNameProvider());
        Assert.AreEqual(typeof(IEnumerable<HeadlessMediaWithCrops>), valueConverter.GetHeadlessPropertyValueType(publishedPropertyType));
        ...
        var result = valueConverter.ConvertIntermediateToHeadlessObject(Mock.Of<IPublishedElement>(), publishedPropertyType, PropertyCacheLevel.Element, inter, false) as IEnumerable<HeadlessMediaWithCrops>;
        Assert.NotNull(result);
    ...
    private Guid SetupMedia(string name, string extension, int width, int height, string altText)
    {
        ...
        void AddProperty(string alias, object value)
        {
            ...
            property.Setup(p => p.GetHeadlessValue(It.IsAny<string?>(), It.IsAny<string?>())).Returns(value);
