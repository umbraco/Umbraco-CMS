...
public class MarkdownEditorValueConverterTests : PropertyValueConverterTests
{
    ...
    public void MarkdownEditorValueConverter_ConvertsValueToMarkdownString(object inter, string expected)
    {
        ...
        Assert.AreEqual(typeof(string), valueConverter.GetContentApiPropertyValueType(Mock.Of<IPublishedPropertyType>()));
        var result = valueConverter.ConvertIntermediateToContentApiObject(Mock.Of<IPublishedElement>(), Mock.Of<IPublishedPropertyType>(), PropertyCacheLevel.Element, inter, false);
