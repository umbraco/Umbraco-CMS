...
public class RichTextPropertyValueHandlerTests : ContentTestBase
{
    ...
    public void RichTextEditor_CanIndexAllTextRelevanceLevels()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(richTextValue.TextsR1, Is.EqualTo(new[] { "H1 Heading #1" }).AsCollection);
            Assert.That(richTextValue.TextsR2, Is.EqualTo(new[] { "H2 Heading #1", "H2 Heading #2" }).AsCollection);
            Assert.That(richTextValue.TextsR3, Is.EqualTo(new[] { "H3 Heading #1", "H3 Heading #2", "H3 Heading #3" }).AsCollection);

            Assert.That(richTextValue.Texts, Is.EqualTo(new[] { "Paragraph #1 Paragraph #2", "Paragraph #3 Paragraph #4", "Paragraph #5", "Paragraph #6", "List Item #1 List Item #2" }).AsCollection);
        });
    }
    ...
    public void RichTextEditor_IncludesTextFromNestedTags()
    {
        ...
        IndexValue? richTextValue = document.Fields.FirstOrDefault(f => f.FieldName == "richTextValue")?.Value;
        Assert.That(richTextValue, Is.Not.Null);
        Assert.That(richTextValue.Texts, Is.EqualTo(new[] { "Some bold text", "A link to somewhere" }).AsCollection);
    }
    ...
    public void RichTextEditor_IncludesSimpleValues(object value)
    {
        ...
        IndexValue? richTextValue = document.Fields.FirstOrDefault(f => f.FieldName == "richTextValue")?.Value;
        Assert.That(richTextValue, Is.Not.Null);
        Assert.That(richTextValue.Texts, Is.EqualTo(new[] { value.ToString() }).AsCollection);
    }
