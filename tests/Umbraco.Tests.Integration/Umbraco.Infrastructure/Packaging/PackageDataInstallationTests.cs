...
internal sealed class PackageDataInstallationTests : UmbracoIntegrationTestWithContent
{
    ...
    private void AssertCheckBoxListTests(string strXml)
    {
        ...
        Assert.That(
            configuration,
            Is.EqualTo("{\"items\":[\"test\",\"test3\",\"test2\"]}"));
    }
    ...
    public async Task Can_Import_Content_With_Non_Default_Template_Via_TemplateAlias()
    {
        ...
        Assert.That(importedTemplates, Has.Count.EqualTo(2));
        ...
        Assert.That(importedContentTypes, Has.Count.EqualTo(1));
        ...
        Assert.That(expectedTemplate, Is.Not.Null);
        Assert.That(contents, Has.Count.EqualTo(1));
        Assert.That(contents.Single().TemplateId, Is.EqualTo(expectedTemplate.Id));
    }
