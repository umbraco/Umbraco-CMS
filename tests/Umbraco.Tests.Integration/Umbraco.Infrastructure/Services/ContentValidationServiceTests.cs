...
internal sealed class ContentValidationServiceTests : UmbracoIntegrationTestWithContent
{
    ...
    public async Task Can_Validate_Mandatory_Property_For_Default_Segment()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(validationError.Alias, Is.EqualTo("title"));
            Assert.That(validationError.Segment, Is.EqualTo(null));
            Assert.That(validationError.JsonPath, Is.Empty);

            Assert.That(validationError.ErrorMessages, Has.Length.EqualTo(1));
            Assert.That(validationError.ErrorMessages.Single(), Is.EqualTo(Constants.Validation.ErrorMessages.Properties.Missing));
        });
    }
