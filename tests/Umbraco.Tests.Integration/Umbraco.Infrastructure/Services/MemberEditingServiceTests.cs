...
internal sealed class MemberEditingServiceTests : UmbracoIntegrationTest
{
    ...
    public async Task Two_Factor_Providers_Are_Retained_When_Updating_Without_Access()
    {
        ...
        Assert.That(await TwoFactorLoginService.IsTwoFactorEnabledAsync(member.Key), Is.True);
    ...
    public async Task Can_Disable_Two_Factor_Providers_With_Access()
    {
        ...
        Assert.That(await TwoFactorLoginService.IsTwoFactorEnabledAsync(member.Key), Is.False);
    }
