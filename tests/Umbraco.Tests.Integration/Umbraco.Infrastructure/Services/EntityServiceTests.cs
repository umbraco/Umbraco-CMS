...
internal sealed class EntityServiceTests : UmbracoIntegrationTest
{
    ...
    [Test]
    public void EntityService_Can_Get_Keys_With_Children_For_Document()
    {
        var result = EntityService.GetKeysWithChildren(
            UmbracoObjectTypes.Document,
            [_textpage.Key, _subpage.Key, _subpage2.Key]);

        Assert.AreEqual(1, result.Count);
        Assert.IsTrue(result.Contains(_textpage.Key));
    }

    [Test]
    public void EntityService_Can_Get_Keys_With_Children_For_Media()
    {
        var result = EntityService.GetKeysWithChildren(
            UmbracoObjectTypes.Media,
            [_folder.Key, _subfolder.Key, _subfolder2.Key, _image.Key]);

        Assert.AreEqual(2, result.Count);
        Assert.IsTrue(result.Contains(_folder.Key));
        Assert.IsTrue(result.Contains(_subfolder.Key));
    }

    [Test]
    public void EntityService_Cannot_Get_Keys_With_Children_Of_Another_Object_Type()
    {
        // Both of these have children, but of the other object type - so neither should be returned.
        var documentsQueriedAsMedia =
            EntityService.GetKeysWithChildren(UmbracoObjectTypes.Media, [_textpage.Key]);
        var mediaQueriedAsDocuments =
            EntityService.GetKeysWithChildren(UmbracoObjectTypes.Document, [_folder.Key, _subfolder.Key]);

        Assert.IsEmpty(documentsQueriedAsMedia);
        Assert.IsEmpty(mediaQueriedAsDocuments);
    }
    ...
    [Test]
    public void EntityService_Cannot_Get_Keys_With_Children_For_Parents_Of_Another_Object_Type()
    {
        // Sub container 2 is a DocumentTypeContainer whose only child is a DocumentType. Asking for
        // DocumentType keys must not return it - it has a child of that type, but is not one itself.
        var result = EntityService.GetKeysWithChildren(
            UmbracoObjectTypes.DocumentType,
            [_documentTypeSubContainer2Key]);

        Assert.IsEmpty(result);
    }

    [Test]
    public void EntityService_Can_Get_Keys_With_Children_Matching_Tree_Has_Children_For_Containers()
    {
        // Pins the implementation to the default interface implementation (and so to the tree's own
        // definition of HasChildren) for a key set mixing containers with the items inside them. The
        // containers hold document types, so an implementation that only filtered the children by
        // object type would wrongly return them here.
        Guid[] keys =
        [
            _documentTypeRootContainerKey,
            _documentTypeSubContainer1Key,
            _documentTypeSubContainer2Key,
            _documentType1Key,
        ];

        var result = EntityService.GetKeysWithChildren(UmbracoObjectTypes.DocumentType, keys);
        var fromEntitySlims = EntityService
            .GetAll(UmbracoObjectTypes.DocumentType, keys)
            .Where(entity => entity.HasChildren)
            .Select(entity => entity.Key)
            .ToHashSet();

        CollectionAssert.AreEquivalent(fromEntitySlims, result);
    }

    [Test]
    public void EntityService_Can_Get_Keys_With_Children_Matching_Tree_Has_Children()
    {
        Guid[] keys = [_textpage.Key, _subpage.Key, _subpage2.Key, _trashed.Key];

        var result = EntityService.GetKeysWithChildren(UmbracoObjectTypes.Document, keys);
        var fromEntitySlims = EntityService
            .GetAll(UmbracoObjectTypes.Document, keys)
            .Where(entity => entity.HasChildren)
            .Select(entity => entity.Key)
            .ToHashSet();
    ...
    [Test]
    public void EntityService_Can_Get_Keys_With_Children_Across_Parameter_Batches()
    {
        // Exceed the per-statement parameter budget so the batching loop has to run more than once.
        var keys = Enumerable
            .Range(0, Constants.Sql.MaxParameterCount + 10)
            .Select(_ => Guid.NewGuid())
            .Append(_textpage.Key)
            .ToArray();
