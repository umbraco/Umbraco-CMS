...
internal sealed partial class ContentTypeServiceTests : UmbracoIntegrationTest
{
    ...
    public async Task GetComposedOf_Distinguishes_Composition_From_Inheritance()
    {
        ...
        var composedOfIds = (await ContentTypeService.GetComposedOfAsync(parent.Id)).Select(x => x.Id).ToArray();
        var compositionIds = (await ContentTypeService.GetComposedOfAsync(parent.Id, ComposedOfType.Composition)).Select(x => x.Id).ToArray();
        var inheritanceIds = (await ContentTypeService.GetComposedOfAsync(parent.Id, ComposedOfType.Inheritance)).Select(x => x.Id).ToArray();
        var allIds = (await ContentTypeService.GetComposedOfAsync(parent.Id, ComposedOfType.All)).Select(x => x.Id).ToArray();
