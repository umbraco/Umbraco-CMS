...
internal sealed class TemplateRepositoryTest : UmbracoIntegrationTest
{
    ...
    public async Task Can_Perform_Delete_When_Assigned_To_Doc()
    {
        ...
        using (provider.CreateScope())
        {
            ...
            var contentTypeRepository = new ContentTypeRepository(scopeAccessor, AppCaches.Disabled, LoggerFactory.CreateLogger<ContentTypeRepository>(), commonRepository, languageRepository, ShortStringHelper, Mock.Of<IRepositoryCacheVersionService>(), IdKeyMap, Mock.Of<ICacheSyncService>(), newScopeAccessor);
            ...
            var relationRepository = new RelationRepository(newScopeAccessor, LoggerFactory.CreateLogger<RelationRepository>(), relationTypeRepository, Mock.Of<IRepositoryCacheVersionService>(), Mock.Of<ICacheSyncService>());
            var propertyEditors =
