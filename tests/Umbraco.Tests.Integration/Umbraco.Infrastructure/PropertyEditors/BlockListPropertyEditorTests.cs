...
internal sealed class BlockListPropertyEditorTests : UmbracoIntegrationTest
{
    ...
    public async Task Can_Track_Tags_For_Block_Level_Variance()
    {
        ...
        await ContentTypeService.CreateAsync(blockListContentType, Constants.Security.SuperUserKey);
    ...
    public async Task Can_Handle_Missing_Layout_Keys(bool useLayoutKey)
    {
        ...
        Assert.That(blockListLayoutItems, Is.Not.Null);
        Assert.That(blockListLayoutItems, Has.Length.EqualTo(1));
        Assert.That(blockListLayoutItems.First().Key, Is.EqualTo(useLayoutKey ? layoutKey : contentElementKey));
    }
