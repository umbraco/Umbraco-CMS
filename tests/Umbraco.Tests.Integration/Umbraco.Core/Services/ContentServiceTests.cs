...
internal sealed partial class ContentServiceTests : UmbracoIntegrationTestWithContent
{
    ...
    public void Can_Get_All_Versions_Of_Content()
    {
        ...
        Assert.That(
            versions[0].GetValue("author", published: true),
            Is.EqualTo("Bob Hope")); // and the published published value
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public async Task Unpublish_All_Cultures_Has_Unpublished_State()
    {
        ...
        Assert.That(content.IsCulturePublished(langUk.IsoCode), Is.False);
        Assert.That(
            content.PublishedState,
            Is.EqualTo(PublishedState.Unpublished)); // the last culture was unpublished so the document should also reflect this
    ...
    public void Can_SaveAndPublish_Invariant_Content_Without_Prior_Save()
    {
        ...
        Assert.That(retrieved.GetValue<string>("author"), Is.EqualTo("Test Author"));
        Assert.That(retrieved.Published, Is.True);
    }
    ...
    public void SaveAndPublish_Fires_Notifications()
    {
        ...
        ContentNotificationHandler.SavingContent = notification =>
        {
            ...
            Assert.That(notification.SavedEntities.Count(), Is.EqualTo(1));
            var entity = notification.SavedEntities.First();
            Assert.That(entity.Name, Is.EqualTo(contentName));
        };
    ...
    public void SaveAndPublish_Can_Be_Cancelled_By_Saving_Notification()
    {
        ...
        try
        {
            ...
            Assert.That(result.Success, Is.False);
            Assert.That(result.Result, Is.EqualTo(PublishResultType.FailedPublishCancelledByEvent));
            Assert.That(content.Published, Is.False);
        }
        finally
        {
    ...
    public void Cannot_SaveAndPublish_Expired_Content()
    {
        ...
        Assert.That(result.Success, Is.False);
        Assert.That(content.Published, Is.False);
        Assert.That(result.Result, Is.EqualTo(PublishResultType.FailedPublishHasExpired));
    }
    ...
    public async Task Cannot_SaveAndPublish_Expired_Culture()
    {
        ...
        Assert.That(result.Success, Is.False);
        Assert.That(result.Result, Is.EqualTo(PublishResultType.FailedPublishCultureHasExpired));
        Assert.That(content.Published, Is.False);
    }
    ...
    public async Task Cannot_SaveAndPublish_Culture_Awaiting_Release()
    {
        ...
        Assert.That(result.Success, Is.False);
        Assert.That(result.Result, Is.EqualTo(PublishResultType.FailedPublishCultureAwaitingRelease));
        Assert.That(content.Published, Is.False);
    }
    ...
    public void Can_Get_Published_Descendant_Versions()
    {
        ...
        Assert.That(publishedDescendants.Count, Is.Not.EqualTo(0));
    ...
    public void Can_Rollback_Version_On_Content()
    {
        ...
        Assert.That(
            ContentService.GetById(content.Id).GetValue<string>("author"),
            Is.EqualTo("Francis Doe")); // version2 author is Francis
        ...
        Assert.That(
            ContentService.GetById(content.Id).GetValue<string>("author"),
            Is.EqualTo("Jane Doe")); // version3 author is Jane
        ...
        Assert.That(
            ContentService.GetById(content.Id).GetValue<string>("author"),
            Is.EqualTo("Francis Doe")); // author is now Francis again
    ...
    public async Task Ensure_Unique_Culture_Names()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            ...
            Assert.That(
                ...
                Is.EqualTo("child" + (i == 0 ? string.Empty : " (" + i + ")")));
    ...
    public async Task Can_SaveRead_Variations()
    {
        ...
        Assert.That(content2.GetValue("prop", langFr.IsoCode, published: true), Is.Null);
        Assert.That(
            content2.GetValue("prop", langUk.IsoCode, published: true),
            Is.EqualTo("value-uk1")); // has value, see note above
