...
internal sealed partial class ContentTypeEditingServiceTests
{
    ...
    [Test]
    public async Task Can_Add_New_Composition_While_Keeping_Existing_Composition_That_Already_Collides_With_Descendant()
    {
        ...
        // simulate a pre-existing (e.g. pre-dating this validation) grandfathered collision: attach B to Parent
        // directly through IContentTypeService, bypassing ContentTypeEditingService's own validation entirely
        ...
        // re-fetch Parent so ContentTypeComposition reflects B, attached above via the raw service - the
        // original `parent` object was loaded before that call, and passing it stale to UpdateAsync would
        // make ValidateCompositions treat B as newly-introduced too, defeating the point of this test
        parent = ContentTypeService.Get(parent.Key)!;

        // verify the grandfathered precondition actually took hold before testing against it
        Assert.IsTrue(parent.ContentTypeComposition.Any(c => c.Key == compositionB.Key), "Precondition failed: Composition B was not persisted on Parent.");

        var compositionC = (await ContentTypeEditingService.CreateAsync(CompositionModelWithProperty("Composition C", "otherAlias"), Constants.Security.SuperUserKey)).Result!;
        var updateModel = ContentTypeUpdateModel(
            "Parent",
            alias: parent.Alias,
            compositions:
            [
                new() { CompositionType = CompositionType.Composition, Key = compositionB.Key },
                new() { CompositionType = CompositionType.Composition, Key = compositionC.Key },
            ]);

        var result = await ContentTypeEditingService.UpdateAsync(parent, updateModel, Constants.Security.SuperUserKey);

        // B's pre-existing (grandfathered) collision must not block adding the unrelated, non-colliding C
        Assert.IsTrue(result.Success, result.Status.ToString());
    }

    [Test]
    public async Task Cannot_Add_New_Composition_That_Collides_With_Descendant_Even_When_Another_Composition_Already_Collides()
    {
        var compositionB = (await ContentTypeEditingService.CreateAsync(CompositionModelWithProperty("Composition B", "sharedAlias"), Constants.Security.SuperUserKey)).Result!;
        var parent = (await ContentTypeEditingService.CreateAsync(ContentTypeCreateModel("Parent"), Constants.Security.SuperUserKey)).Result!;

        var childContainer = ContentTypePropertyContainerModel();
        var childModel = ContentTypeCreateModel(
            "Child",
            propertyTypes:
            [
                ContentTypePropertyTypeModel("sharedAlias", "sharedAlias", containerKey: childContainer.Key),
                ContentTypePropertyTypeModel("anotherSharedAlias", "anotherSharedAlias", containerKey: childContainer.Key),
            ],
        ...
        // compositionD is genuinely new, and collides with a *different* own property on Child than B does
        var compositionD = (await ContentTypeEditingService.CreateAsync(CompositionModelWithProperty("Composition D", "anotherSharedAlias"), Constants.Security.SuperUserKey)).Result!;
        var updateModel = ContentTypeUpdateModel(
            "Parent",
            alias: parent.Alias,
            compositions:
            [
                new() { CompositionType = CompositionType.Composition, Key = compositionB.Key },
                new() { CompositionType = CompositionType.Composition, Key = compositionD.Key },
            ]);

        var result = await ContentTypeEditingService.UpdateAsync(parent, updateModel, Constants.Security.SuperUserKey);

        // B's pre-existing collision must not mask D's genuinely new, distinct collision
        Assert.IsFalse(result.Success);
        Assert.AreEqual(ContentTypeOperationStatus.InvalidComposition, result.Status);
    }
