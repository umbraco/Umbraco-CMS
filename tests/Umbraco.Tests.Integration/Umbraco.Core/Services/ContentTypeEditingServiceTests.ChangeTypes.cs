...
internal sealed partial class ContentTypeEditingServiceTests
{
    ...
    public async Task Add_Composition_Emits_CompositionAdded()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(payload.ChangeTypes.HasType(ContentTypeChangeTypes.RefreshMain), Is.True, "CompositionAdded should include RefreshMain");
            Assert.That(payload.ChangeTypes.HasType(ContentTypeChangeTypes.RawDataUnaffected), Is.True, "CompositionAdded should be flagged RawDataUnaffected");
        });
    }
}
