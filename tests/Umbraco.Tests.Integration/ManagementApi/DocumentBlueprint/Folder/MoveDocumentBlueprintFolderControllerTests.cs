...
public class MoveDocumentBlueprintFolderControllerTests : ManagementApiUserGroupTestBase<MoveDocumentBlueprintFolderController>
{
    ...
    {
        var folder = await ContentBlueprintContainerService.CreateAsync(Guid.NewGuid(), "TestFolder", Constants.System.RootKey, Constants.Security.SuperUserKey);
        _folderKey = folder.Result!.Key;

        var targetFolder = await ContentBlueprintContainerService.CreateAsync(Guid.NewGuid(), "TestTargetFolder", Constants.System.RootKey, Constants.Security.SuperUserKey);
        _targetFolderKey = targetFolder.Result!.Key;
    }

    protected override Expression<Func<MoveDocumentBlueprintFolderController, object>> MethodSelector =>
