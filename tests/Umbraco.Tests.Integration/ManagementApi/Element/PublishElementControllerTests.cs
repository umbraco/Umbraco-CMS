...
public class PublishElementControllerTests : ManagementApiUserGroupTestBase<PublishElementController>
{
    ...
    protected override Expression<Func<PublishElementController, object>> MethodSelector =>
        x => x.Publish(CancellationToken.None, _elementKey, null!);
    ...
    {
        var publishModel = new PublishElementRequestModel
        {
            PublishSchedules = [new CultureAndScheduleRequestModel { Culture = null }],
        };

        return await Client.PutAsync(Url, JsonContent.Create(publishModel));
    }
}
