...
public class MoveDocumentTypeFolderControllerTests : ManagementApiUserGroupTestBase<MoveDocumentTypeFolderController>
{
    ...
    {
        var folder = await ContentTypeContainerService.CreateAsync(Guid.NewGuid(), "TestFolder", Constants.System.RootKey, Constants.Security.SuperUserKey);
        _folderKey = folder.Result!.Key;

        var targetFolder = await ContentTypeContainerService.CreateAsync(Guid.NewGuid(), "TestTargetFolder", Constants.System.RootKey, Constants.Security.SuperUserKey);
        _targetFolderKey = targetFolder.Result!.Key;
    }

    protected override Expression<Func<MoveDocumentTypeFolderController, object>> MethodSelector =>
