...
public class MoveDataTypeFolderControllerTests : ManagementApiUserGroupTestBase<MoveDataTypeFolderController>
{
    ...
    {
        var folder = await DataTypeContainerService.CreateAsync(Guid.NewGuid(), "TestFolder", Constants.System.RootKey, Constants.Security.SuperUserKey);
        _folderKey = folder.Result!.Key;

        var targetFolder = await DataTypeContainerService.CreateAsync(Guid.NewGuid(), "TestTargetFolder", Constants.System.RootKey, Constants.Security.SuperUserKey);
        _targetFolderKey = targetFolder.Result!.Key;
    }

    protected override Expression<Func<MoveDataTypeFolderController, object>> MethodSelector =>
