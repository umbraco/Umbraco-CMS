...
public abstract class UmbracoIntegrationTestWithMediaEditing : UmbracoIntegrationTest
{
    ...
    {
        // MediaTypeCreateModel = MediaTypeEditingBuilder.CreateBasicFolderMediaType();
        //
        // var mediaTypeFolderAttempt = await MediaTypeEditingService.CreateAsync(MediaTypeCreateModel, Constants.Security.SuperUserKey);
        // Assert.IsTrue(mediaTypeFolderAttempt.Success);

        // Gets all media types
        var mediaTypes = MediaTypeService.GetAll();
        var mediaTypesList = mediaTypes.ToList();
        var folderMediaType = mediaTypesList.FirstOrDefault(x => x.Alias == "Folder");
