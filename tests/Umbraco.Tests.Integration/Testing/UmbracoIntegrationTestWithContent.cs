...
public abstract class UmbracoIntegrationTestWithContent : UmbracoIntegrationTest
{
    ...
    public virtual async Task CreateTestDataAsync()
    {
        ...
        await ContentService.SaveAsync(Subpage, Constants.Security.SuperUserKey, contentSchedule, CancellationToken.None);
        ...
        await ContentService.SaveAsync(Subpage2, Constants.Security.SuperUserKey, null, CancellationToken.None);


        Subpage3 = ContentBuilder.CreateSimpleContent(ContentType, "Text Page 3", Textpage.Id);
        Subpage3.Key = new Guid(SubPage3Key);
        await ContentService.SaveAsync(Subpage3, Constants.Security.SuperUserKey, null, CancellationToken.None);
