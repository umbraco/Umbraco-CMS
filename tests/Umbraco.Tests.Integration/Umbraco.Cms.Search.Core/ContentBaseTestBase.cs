...
public abstract class ContentBaseTestBase : TestBase
{
    ...
    protected void VerifyDocumentSystemValues(TestIndexDocument document, IContentBase content, string[] tags)
    {
        ...
        Assert.Multiple(() =>
        {
            var contentTypeIdValue = document.Fields.FirstOrDefault(f => f.FieldName == Constants.IndexFieldNames.ContentTypeId)?.Value.Keywords?.SingleOrDefault();
            Assert.That(contentTypeIdValue, Is.EqualTo(content.ContentType.Key.AsKeyword()));

            var nameValue = document.Fields.FirstOrDefault(f => f.FieldName == Constants.IndexFieldNames.Name)?.Value.TextsR1?.SingleOrDefault();
            ...
            var objectTypeValue = document.Fields.FirstOrDefault(f => f.FieldName == Constants.IndexFieldNames.ObjectType)?.Value.Keywords?.SingleOrDefault();
            Assert.That(objectTypeValue, Is.EqualTo(expectedObjectTypeValue));

            IEnumerable<string>? tagsValue = document.Fields.FirstOrDefault(f => f.FieldName == Constants.IndexFieldNames.Tags)?.Value.Keywords;
