...
function umbracoEditorCommand(id,commandKey,option) 
  ...
  {
    // Fixes issue with code gets inserted outside the editor in IE
    if (navigator.userAgent.match('MSIE')) {
        if (textareaEditorMode == 'undefined' && document.selection.createRange().parentElement().tagName == 'DIV') {
            alert('FOCUS OUTSIDE OF THE EDITOR\n--------------------------------------------------------------------------\n\nPlease place the cursor where you wish to insert the code by using either mouse or keyboard.');
            return;
        }
    }
    ...
    var dialogTitle = "Insert umbraco field";
    if (tagName == "UMBRACOGETDICTIONARY") 
    {
        dialogTitle = "Insert Dictionary Item";
    }
    top.openModal(path + tagUrl + ".aspx?tagName=" + tagName + "&tagText=" + tagText + "&move=" + move + "&objectId=meh",dialogTitle, height, width);
  }
  

  function umbracoInsertFieldDo(theObjectId, Text, move) 
  {
      //editorSource.insertCode(Text);
      insertCodeAtCaret(document.getElementById(theObjectId), Text)
  }
  
  function xbfInsertAtCaretAndMove (el, txt, move) 
  {
      //editorSource.insertCode(txt);
      insertCodeAtCaret(document.getElementById(theObjectId), txt)
  }
...
///////////////////////////////////////////
  ...
  top.openModal("dialogs/editMacro.aspx","Insert macro", 600, 500);
  //var	fieldTag = nytVindue("../dialogs/editMacro.aspx", 600, 350);
  }
  
  function umbracoInsertMacroDo(fieldTag) {
  	fieldTag = fieldTag.replace(/\'/gi,"&#039;");
    xbfInsertAtCaretAndMove(document.forms[0].TemplateBody, fieldTag)
  }
